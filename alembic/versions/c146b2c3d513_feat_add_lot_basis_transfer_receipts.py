"""Add immutable versioned lot basis-transfer receipts.

Revision ID: c146b2c3d513
Revises: c145b2c3d512
...
revision: str = "c146b2c3d513"
down_revision: str | Sequence[str] | None = "c145b2c3d512"
branch_labels: str | Sequence[str] | None = None
depends_on: str | Sequence[str] | None = None


def upgrade() -> None:
    """Create append-only transfer headers before their ordered allocations."""
    ...
    op.create_table(
        ...
        sa.Column("target_lot_id", sa.String(), nullable=False),
        sa.Column("portfolio_id", sa.String(), nullable=False),
        sa.Column("source_instrument_id", sa.String(), nullable=False),
        sa.Column("source_security_id", sa.String(), nullable=False),
        sa.Column("target_instrument_id", sa.String(), nullable=True),
        sa.Column("transfer_timestamp", sa.DateTime(timezone=True), nullable=False),
        ...
        sa.Column("transferred_cost_local", sa.Numeric(18, 10), nullable=False),
        sa.Column("transferred_cost_base", sa.Numeric(18, 10), nullable=False),
        ...
        sa.CheckConstraint(
            "CAST(transferred_cost_local AS TEXT) NOT IN ('NaN', 'Infinity', '-Infinity') "
            "AND CAST(transferred_cost_base AS TEXT) NOT IN ('NaN', 'Infinity', '-Infinity')",
            name="ck_lot_basis_transfer_receipt_amounts_finite",
        ),
        sa.CheckConstraint(
        ...
        sa.ForeignKeyConstraint(
            ...
            name="fk_lot_basis_transfer_receipt_source_tx",
            ondelete="RESTRICT",
        ),
        sa.ForeignKeyConstraint(
            ["portfolio_id"],
            ["portfolios.portfolio_id"],
            name="fk_lot_basis_transfer_receipt_portfolio",
            ondelete="RESTRICT",
        ),
        sa.ForeignKeyConstraint(
            ["source_security_id"],
            ["instruments.security_id"],
            name="fk_lot_basis_transfer_receipt_source_security",
            ondelete="RESTRICT",
        ),
        sa.PrimaryKeyConstraint("id", name="pk_lot_basis_transfer_receipts"),
        sa.UniqueConstraint(
            "receipt_id",
            "receipt_version",
    ...
    op.create_index(
        ...
        [
            "portfolio_id",
            "source_security_id",
            sa.text("transfer_timestamp DESC"),
    ...
    op.create_index(
        ...
        "lot_basis_transfer_receipts",
        ["source_transaction_id", sa.text("receipt_version DESC")],
    )
    op.create_index(
    ...
    op.create_table(
        ...
        sa.Column("retained_quantity", sa.Numeric(18, 10), nullable=False),
        sa.Column("source_cost_local_before", sa.Numeric(18, 10), nullable=False),
        sa.Column("source_cost_base_before", sa.Numeric(18, 10), nullable=False),
        sa.Column("transferred_cost_local", sa.Numeric(18, 10), nullable=False),
        sa.Column("transferred_cost_base", sa.Numeric(18, 10), nullable=False),
        sa.Column("retained_cost_local", sa.Numeric(18, 10), nullable=False),
        sa.Column("retained_cost_base", sa.Numeric(18, 10), nullable=False),
        ...
        sa.CheckConstraint(
            "CAST(retained_quantity AS TEXT) NOT IN ('NaN', 'Infinity', '-Infinity') "
            "AND CAST(source_cost_local_before AS TEXT) NOT IN ('NaN', 'Infinity', '-Infinity') "
            "AND CAST(source_cost_base_before AS TEXT) NOT IN ('NaN', 'Infinity', '-Infinity') "
            "AND CAST(transferred_cost_local AS TEXT) NOT IN ('NaN', 'Infinity', '-Infinity') "
            "AND CAST(transferred_cost_base AS TEXT) NOT IN ('NaN', 'Infinity', '-Infinity') "
            "AND CAST(retained_cost_local AS TEXT) NOT IN ('NaN', 'Infinity', '-Infinity') "
            "AND CAST(retained_cost_base AS TEXT) NOT IN ('NaN', 'Infinity', '-Infinity')",
            name="ck_lot_basis_transfer_allocation_amounts_finite",
        ),
        sa.CheckConstraint(
        ...
        sa.PrimaryKeyConstraint("id", name="pk_lot_basis_transfer_allocations"),
        sa.UniqueConstraint(
            "receipt_id",
            "receipt_version",
            "allocation_ordinal",
    ...
    op.create_index(
        ...
        "lot_basis_transfer_allocations",
        ["portfolio_id", "source_security_id", "source_lot_id", "source_acquisition_date"],
    )


def downgrade() -> None:
    """Remove child allocations before transfer receipt headers."""

    op.drop_index(
        "ix_lot_basis_transfer_allocation_source",
        table_name="lot_basis_transfer_allocations",
    )
    op.drop_table("lot_basis_transfer_allocations")
    op.drop_index(
        "ix_lot_basis_transfer_receipt_target_tx_version",
        table_name="lot_basis_transfer_receipts",
    )
    op.drop_index(
        "ix_lot_basis_transfer_receipt_source_tx_version",
        table_name="lot_basis_transfer_receipts",
    ...
    op.drop_table("lot_basis_transfer_receipts")
