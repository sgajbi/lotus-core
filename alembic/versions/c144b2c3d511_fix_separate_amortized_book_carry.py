...
def _require_acquisition_basis_evidence() -> None:
    ...
    op.execute(
        ...
        "OR EXISTS ("
        "SELECT 1 FROM transactions AS basis_mutation "
        "WHERE basis_mutation.portfolio_id = lot.portfolio_id "
        "AND basis_mutation.security_id = lot.security_id "
        "AND basis_mutation.transaction_type IN ('SPIN_OFF', 'DEMERGER_OUT') "
        "AND basis_mutation.quantity = 0 "
        "AND basis_mutation.transaction_date >= source.transaction_date"
        "))"
        ") THEN "
        "RAISE EXCEPTION "
