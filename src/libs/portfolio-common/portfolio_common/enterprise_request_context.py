...
def request_header_value(request: Request, name: str, default: str) -> str:
...
def request_trace_id(request: Request) -> str | None:
    ...
    extracted_trace_id = trace_id_from_traceparent(
        cast(str | None, request.headers.get("traceparent"))
    )
    if extracted_trace_id is not None:
        return extracted_trace_id

    header_trace_id = normalize_trace_id(cast(str | None, request.headers.get("X-Trace-ID")))
