...
class CorporateActionExecutionPlan:
    """Carry exact source and observation authority for one ordered release attempt."""
    ...
    def execution_plan_hash(self) -> str:
        """Bind the exact manifest, readiness boundary, and canonical release order."""
...
def resolve_corporate_action_execution_gate(
    ...
    if decision.readiness_status is not CorporateActionManifestReadinessStatus.READY:
        if decision.manifest_content_hash is not None or decision.ordered_transaction_ids:
