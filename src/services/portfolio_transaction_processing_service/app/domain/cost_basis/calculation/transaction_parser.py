...
class CostTransactionParser:
    ...
    def __init__(self, error_reporter: CostCalculationErrorCollector) -> None:
    ...
    def parse_transactions(
        ...
        for raw_txn_data in raw_transaction_data:
            ...
            try:
                ...
                error_reason = f"Validation error: {str(exc)}"
                ...
                error_reason = f"Unexpected parsing error: {type(exc).__name__}: {str(exc)}"
                self._error_reporter.add_error(transaction_id, error_reason)
                stub_txn = self._create_stub_transaction(raw_txn_data, error_reason)
                parsed_transactions.append(stub_txn)
    ...
    def _create_stub_transaction(
        self, raw_data: dict[str, Any], error_reason: str
