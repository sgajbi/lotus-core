...
class FinancialReconciliationCompletionConsumer(BaseConsumer):
    ...
    async def process_message(self, msg: Message):
        ...
        try:
            event = FinancialReconciliationCompletedEvent.model_validate(json.loads(value))
            ...
            logger.error(
                "Invalid financial reconciliation completion payload; sending to DLQ.",
                exc_info=True,
            )
            await self._send_to_dlq_async(msg, ValueError("invalid payload"))
        except (DBAPIError, IntegrityError):
            logger.warning("DB error in reconciliation completion consumer; retrying.")
            raise
