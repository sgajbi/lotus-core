...
@dataclass(frozen=True)
class CostCalculationStageResult:
...
class CostCalculationEventProcessor:
    ...
    async def process_valid_event(
        ...
        *,
    ...
    ) -> CostCalculationStageResult | None:
        ...
        return await self.stage_valid_event(
            event=event,
    ...
    async def stage_valid_event(
        ...
        *,
        ...
        repo: CostCalculatorRepository,
        outbox_repo: OutboxRepository,
    ) -> CostCalculationStageResult:
        """Stage cost writes and outbox events inside the caller-owned transaction."""
        ...
        instrument = await repo.get_instrument(prepared_event.security_id)
        ...
        return CostCalculationStageResult(
            emitted_events=tuple(emitted_events),
            instrument_event_count=len(instrument_events_to_publish),
        )
