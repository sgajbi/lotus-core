"""Public contracts for governed benchmark market-series input windows."""
...
def _normalize_series_fields(series_fields: list[str]) -> list[str]:
    ...
    invalid = sorted(
        field
        for field in set(requested_fields)
...
class BenchmarkMarketSeriesRequest(BaseModel):
    """Request native benchmark component market inputs for a bounded window."""
...
class SeriesPoint(BaseModel):
    """One native benchmark component market-series point."""
    ...
    index_price: Decimal | None = Field(
        None, description="Index price value when requested.", examples=["4567.1234000000"]
    )
    index_return: Decimal | None = Field(
        None, description="Index return value when requested.", examples=["0.0023000000"]
    )
    ...
    fx_rate: Decimal | None = Field(
        None,
        description=(
            "Benchmark-currency to target-currency FX context rate when target currency is "
            "requested. This is not component-to-benchmark normalization."
        ),
        examples=["1.0842000000"],
    )
    quality_status: str | None = Field(
        None, description="Quality status for this point.", examples=["accepted"]
    )
...
class ComponentSeriesResponse(BaseModel):
    """Market-series points for one benchmark component index."""
...
class BenchmarkMarketSeriesResponse(SourceDataProductRuntimeMetadata):
    """Native component market inputs with deterministic paging and source proof."""
    ...
    normalization_policy: str = Field(
        ...,
        description=(
            "Contract policy label describing how downstream consumers should interpret the "
            "series. Current policy returns native component series and requires downstream "
            "benchmark-currency normalization."
        ),
    ...
    component_metadata_policy: str = Field(
        ...,
        description=(
            "Contract guidance for resolving canonical component metadata such as classification "
            "labels. Benchmark market-series returns raw component series; use `POST "
            "/integration/indices/catalog` with targeted `index_ids` when canonical component "
            "metadata is required alongside these series."
        ),
