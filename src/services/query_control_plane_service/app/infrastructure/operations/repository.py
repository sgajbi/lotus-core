...
class OperationsRepository:
    ...
    async def get_latest_financial_reconciliation_control_stage(
        ...
        if row is None:
            return None
        return PortfolioControlStageEvidence(
            stage_id=row.id,
    ...
    async def get_lineage_keys(
        ...
        return [
            LineageKeyEvidence(
                ...
                reprocessing_status=row["reprocessing_status"],
                latest_position_history_date=row["latest_position_history_date"],
                latest_daily_snapshot_date=row["latest_daily_snapshot_date"],
                latest_valuation_job_date=row["latest_valuation_job_date"],
                latest_valuation_job_id=row["latest_valuation_job_id"],
                latest_valuation_job_status=row["latest_valuation_job_status"],
                latest_valuation_job_correlation_id=(row["latest_valuation_job_correlation_id"]),
            )
            for row in rows
        ]
