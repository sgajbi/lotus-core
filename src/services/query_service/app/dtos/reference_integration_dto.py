...
class PortfolioTaxLotWindowRequest(BaseModel):
    ...
    lot_status_filter: Literal["OPEN", "CLOSED"] | None = Field(
        None,
        description="Optional explicit lot status filter. When omitted, open lots are returned by default.",
        examples=["OPEN"],
    )
...
class PortfolioTaxLotWindowSupportability(BaseModel):
    ...
    requested_security_count: int | None = Field(
        None,
        description="Number of securities explicitly requested, null when the full portfolio was requested.",
        examples=[2],
    )
...
class MarketDataCoverageRequest(BaseModel):
    ...
    max_staleness_days: int = Field(
        ...
        ge=0,
        le=31,
        description="Maximum acceptable age in calendar days before an observation is marked stale.",
        examples=[5],
    )
    tenant_id: str | None = Field(
        None,
