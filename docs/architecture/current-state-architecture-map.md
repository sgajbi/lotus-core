...
| --- | --- | --- | --- |
| `cost_calculator_service` | `src/services/calculators/cost_calculator_service` | Transitional cost workflow, processor, repository, and compatibility tests behind target ports; cost-basis policy and checkpoint state are target-owned; old replay delivery is removed | Extract processor observability plus target application/infrastructure ports, migrate delivery tests, then delete the legacy root after canonical QA. |
