...
| --- | --- | --- | --- |
| `cost_calculator_service` | `src/services/calculators/cost_calculator_service` | Transitional cost workflow/checkpoint/repository adapters behind target ports; pure cost-basis domain is target-owned; old replay delivery is removed | Extract target application/infrastructure ports, migrate delivery tests, then delete the legacy root after canonical QA. |
