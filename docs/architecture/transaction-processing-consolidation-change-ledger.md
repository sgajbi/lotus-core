...
| Cost-history runtime complexity | partial | CR-1466 removes the FX N+1 pattern; CR-1467 removes FIFO availability scans; CR-1468 makes AVCO source allocation linear; CR-1471 removes full-history replay from strictly ordered safe appends. At 8,000 rows, pure-engine ordered opening is sub-millisecond, disposal is 58-120ms, and backdated rebuild is about 316-318ms. Large open-source-lot restoration remains the measured disposal hotspot. | Query-count/explain/end-to-end load evidence meets target without weakening deterministic replay; optimize durable FIFO/AVCO restore state with exact quantity/local/base parity. |
