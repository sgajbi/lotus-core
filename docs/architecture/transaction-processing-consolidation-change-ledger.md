...
| Replay request path | required | CR-1450 through CR-1452 implement the use case, port, adapter, and production composition. Add one target replay-request consumer that republishes canonical transactions to `transactions.persisted`; combined normal consumer processes replayed transactions under epoch/semantic fences. | Replay ordering, duplicate, partial-publish, epoch, throttle, and backlog tests pass. |
