...
| Legacy normal/replay delivery classes | partial | Position normal delivery and cost replay-request delivery are deleted. CR-1489 isolates active cost workflow code and confines its compatibility consumer; cashflow workflow and delivery still share one file. | Migrate legacy cost delivery tests and split cashflow workflow ownership, then delete both compatibility consumers while retaining target delivery and replay proof. |
