...
| Canonical booked-transaction replay infrastructure adapter | implemented-local | CR-1451 provides short-lived SQLAlchemy session ownership, canonical replayer delegation, correlation propagation, and zero/one cardinality enforcement; CR-1587 gives the capability a domain-owned package and removes flat/root compatibility surfaces. |
| Booked-transaction replay production composition | implemented-local | CR-1452 provides a named canonical repository factory, shared producer, fresh request session, and explicit dependency injection; CR-1591 gives live, replay, and AVCO-reconciliation builders explicit runtime composition ownership. |
...
| Duplicate and repair replay semantic parity | implemented-local | CR-1491 keeps ordinary semantic duplicates as no-ops; CR-1495 adds an internal repair intent plus physical delivery fence so canonical replay can restore missing or stale derived state once without allowing semantic conflicts or broker redelivery to repeat work. Load proof measures ordinary duplicates as `duplicate` and completed canonical repairs as `processed`. |
| Position recalculation single-writer | implemented-local | CR-1492 proves transaction-scoped serialization for one normalized portfolio/security/epoch, exact no-loss/no-duplicate final history, unrelated security/epoch concurrency, bounded lock-wait/coalescing metrics, and dashboard visibility. |
| Cost-basis lot-state single writer | implemented-local | CR-1493 serializes FIFO, AVCO, backdated, replay, first-write, and historical-repair transitions for one normalized portfolio/security key; PostgreSQL proof shows concurrent BUY/SELL/replay convergence and unrelated-security parallelism with bounded contention telemetry. |
| Backdated position trigger coalescing | implemented-local | CR-1494 adds indexed current-epoch transaction lineage detection so a committed burst produces one rebuild and later already-materialized triggers do zero work; PostgreSQL proof shows one epoch advance, exact three-row state, and zero replay fan-out. |
...
| Backdated transaction readiness epoch fence | implemented-local | CR-1478 established the epoch invariant; CR-1624 moves its final ownership into the transaction-processing readiness module and removes the obsolete pipeline consumer/service/repository copy. |
...
| Bounded module outcome and latency metrics | implemented-local | CR-1461 adds the observer port and stable bounded metrics; CR-1589 moves aggregate runtime telemetry under `infrastructure/transaction_processing` and corporate-action basis telemetry under `infrastructure/cost_basis`, removes flat/root compatibility surfaces, and preserves metric names and labels. |
...
| Transaction-processing dashboard | implemented-local | CR-1485; one focused app-local dashboard covers separate live/replay lag, stage p95/failures, async pool state, and outbox backlog. Broad portfolio service filters include the unified job and exclude the three retired workers. Threshold alerts remain evidence-gated. |
...
| Backdated cost correction persistence | implemented-local | CR-1463 atomically persists the deterministic affected suffix while publishing only the incoming event. CR-1696 additionally refreshes calculated prefix rows that lack durable base/local economics or Core calculation lineage before position staging, closing the unfavorable concurrent lock-order gap without republishing or rewriting governed history. CR-1464 proves FIFO, AVCO, fee-bearing multi-lot, cross-currency local/base, rollback, position epoch/basis, fee cardinality, and event count. | Runtime capacity and cutover gates remain; incremental processing keeps the suffix boundary, while every full rebuild must durably fill missing economics authority for the position timeline it consumes and retain history-depth-independent statement count for governed rows. |
...
| Pipeline orchestrator transaction-stage subscriptions/state | removed-local | CR-1624 removes the final atomic-completion subscription plus transaction-specific handler, service, policy, event-factory, repository, and test paths. Portfolio aggregation/reconciliation coordination remains. | Define retention/archive policy for historical transaction-stage rows; do not restore the retired consumer or group. |
...
| Historical transaction-stage rows/keys in pipeline stage state | retention-candidate | No new transaction-stage rows are written by the pipeline service. Define retention/archive policy before schema/row cleanup; surviving portfolio-control rows still use the table. |
