...
| Backdated pipeline readiness epoch fence | implemented-local | CR-1478; exact transaction-stage advisory locking serializes outbox and inline-rebuild registration, accepts current/newer epochs, and rejects stale pre-bump completion without emitting superseded transaction or valuation readiness. |
