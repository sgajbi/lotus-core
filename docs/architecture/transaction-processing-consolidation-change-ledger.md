...
| Atomic app-local/CI runtime cutover | implemented-local | CR-1487; exact Kafka live/replay offset handoff, one Compose/CI worker, one Prometheus scrape, project-independent load diagnostics, completed transaction load evidence, and clean bank-day downstream tie-out. Registry/Kubernetes cutover remains pending. |
...
| Throughput and capacity | partial | CR-1471 adds deterministic engine-only modes; CR-1482 proves bounded replay ordering/drain; CR-1487 fixes the false submission-only load gate and records clean completed-domain throughput/drain plus exact bank-day downstream tie-out. Still measure p50/p95/p99, pool wait/utilization, lag/failure recovery, shutdown-under-load, and a reviewed three-service comparison. | No material regression; bounded in-flight and per-portfolio ordering proven. |
...
| CI service inventories | implemented-local | CR-1460 enrolls prebuild/image release; CR-1465 adds the blocking PostgreSQL suite; CR-1487 atomically replaces all three legacy workers in Docker/E2E/latency/performance/failure-recovery sets. | GitHub image/release and mainline runtime gates pass. |
...
| Load/support scripts | implemented-local | CR-1487 seeds valid source records, requires completed cost/cashflow/position/idempotency evidence, bounds fixtures to the run date, and resolves logs by Compose project/service. Clean fast load and bank-day DB/API/log tie-out passed. | Retain through full load, failure recovery, canonical QA, and platform publication. |
...
|---|---|---|
| README and supported architecture docs | partial | App-local/CI two-consumer runtime, module ownership, port, offset handoff, and current pending release/Kubernetes work are published in CR-1487. Final production topology follows remaining cutover. |
