...
| Throughput and capacity | partial | CR-1471 adds deterministic engine-only opening/disposal/backdated mode evidence and bounded mode/restore metrics. CR-1482 proves bounded in-flight replay ordering/backlog and correct local drain lifecycle. Measure database and Kafka end-to-end events/second, p50/p95/p99, pool utilization, query count, lag, failure recovery, and deployed shutdown drain against the three-service baseline. | No material regression; bounded in-flight and per-portfolio ordering proven. |
| Aggregate observability | partial | CR-1457 makes task exits attributable, CR-1458 proves readiness/build metadata, CR-1461 adds bounded module outcome/error/latency metrics, and CR-1483 adds committed partition lag. Shared outbox gauges already expose pending, retry-eligible, retry-waiting, terminal-failed, and oldest-pending state. Add DB pool diagnostics, traces, target dashboard/alerts, and final support runbook. | Observability contract and failure-injection tests pass. |
