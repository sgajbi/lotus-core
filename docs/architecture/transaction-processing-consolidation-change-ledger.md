...
| Replay request path | required | CR-1450/CR-1451 implement the application use case, port, and adapter. Add production composition and one target replay-request consumer that republishes canonical transactions to `transactions.persisted`; combined normal consumer processes replayed transactions under epoch/semantic fences. | Replay ordering, duplicate, partial-publish, epoch, throttle, and backlog tests pass. |
...
| Target image | required | Add one production Dockerfile/dependency definition and OCI/version provenance for `portfolio_transaction_processing_service`. | Image SHA label/tag, repo/branch/build/run metadata, digest manifest, SBOM, scan, signature, attestation, no build secrets. |
| Compose/runtime manifests | required | Add target service by digest-capable image reference and remove three runtime services atomically. | Canonical QA and rollback deployment proof pass. |
...
| CI service inventories | required | Update `scripts/ci_service_sets.py`, `scripts/prebuild_ci_images.py`, image release matrix, test manifest, and changed-service mapping. | Local CI lane and GitHub image/release gates pass. |
