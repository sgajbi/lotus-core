...
| Cost-history runtime complexity | partial | CR-1466 removes the FX N+1 pattern: `N` cross-currency timeline rows now require `P` bounded reads for `P` distinct normalized pairs. CR-1467 removes repeated FIFO open-lot scans and adds a reproducible engine profile. AVCO remains quadratic because every disposal reconciles every historical source contribution. Characterize database plans and engine CPU/memory under governed long histories; change incremental state only with FIFO, AVCO, backdated, fee/FX, multi-lot, and corporate-action parity. | AVCO scaling is remediated; query-count/explain/load evidence meets target without weakening deterministic replay; otherwise retain full-history correctness with explicit capacity limits and diagnostics. |
