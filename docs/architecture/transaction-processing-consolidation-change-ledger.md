...
| Replay request path | required | CR-1450 through CR-1453 implement the use case, port, adapter, composition, request mapper, and target consumer. Compose exactly one target replay-request consumer beside the combined normal consumer without activating the target runtime yet. | Replay ordering, duplicate, partial-publish, epoch, throttle, and backlog tests pass. |
