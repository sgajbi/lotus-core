...
| Aggregate observability | partial | CR-1457 makes task exits attributable, CR-1458 proves readiness/build metadata, CR-1461 adds bounded module outcome/error/latency metrics, CR-1483 adds committed partition lag, and CR-1484 adds async DB pool state. Shared outbox gauges expose pending, retry-eligible, retry-waiting, terminal-failed, and oldest-pending state. Add traces, target dashboard/alerts, and final support runbook. | Observability contract and failure-injection tests pass. |
