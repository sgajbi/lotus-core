...
| Backdated cost correction persistence | required | Full ordered history is index-supported and intentionally retained for FIFO/AVCO correctness, but the workflow recomputes later transactions in memory while `_persist_new_processed_transactions` filters writes/events to the incoming transaction ID. Prove whether later realized/cost fields require correction, then persist and publish the deterministic affected suffix atomically or document the existing replay owner with equivalent evidence. | Backdated BUY-before-SELL and multi-lot/FX/fee tests prove current and later transaction costs, realized P&L, outbox events, duplicate replay, and rollback behavior; no stale future cost rows remain. |
