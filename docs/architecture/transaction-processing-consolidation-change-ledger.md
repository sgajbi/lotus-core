...
| Historical AVCO lot evidence | required | CR-1479 establishes current aggregate state on deterministic rebuild and keeps new ordered source rows reconciled. Reconcile/backfill historical AVCO `position_lot_state` plus `average_cost_pool_state` before treating tax-lot source products as current after cutover. | Idempotent migration, row-count/value reconciliation, rollback, and source-product supportability evidence pass. |
