...
| Replay request path | required | CR-1450 implements the application use case/port. Add one target replay-request consumer and adapter that republishes canonical transactions to `transactions.persisted`; combined normal consumer processes replayed transactions under epoch/semantic fences. | Replay ordering, duplicate, partial-publish, epoch, throttle, and backlog tests pass. |
