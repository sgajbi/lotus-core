...
| Bounded FIFO disposal restoration | implemented-local | CR-1473; ordered FIFO consumption streams only quantity-covering source lots, updates that explicit subset, fails on missing selected state, and leaves omitted later lots unchanged. AVCO and rebuilds retain complete snapshots. |
...
| Cost-history runtime complexity | partial | CR-1466 removes the FX N+1 pattern; CR-1467 removes FIFO availability scans; CR-1468 makes AVCO source allocation linear; CR-1471 removes full-history replay from strictly ordered safe appends; CR-1473 bounds FIFO disposal restoration. At 8,000 rows, FIFO disposal restores 1 lot at 0.065ms while AVCO restores 6,000 at 112.493ms; backdated rebuild remains about 260-279ms. | Query-count/explain/end-to-end load evidence meets target without weakening deterministic replay; optimize durable AVCO restore state with exact quantity/local/base parity. |
