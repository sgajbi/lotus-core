...
| Mixed corporate-action cash economics | implemented-local | CR-1472; true cash consideration carries allocated local/base basis, realized capital/FX/total P&L, `CORPORATE_ACTION_PROCEEDS`, linked cash settlement, target-plus-cash basis reconciliation, and combined PostgreSQL rollback proof. Cash-in-lieu remains separate follow-up scope. |

## Required Before Runtime Cutover

| Surface | Status | Required change | Removal/cutover prerequisite |
|---|---|---|---|
| Concrete BUY/SELL and multi-leg behavior | required | Baseline FIFO partial disposal is implemented in CR-1442; fee-aware full disposal in CR-1443; effective-dated cross-currency valuation in CR-1444; AVCO source/pool reconciliation in CR-1448; FIFO multi-lot selection in CR-1449; mixed demerger cash basis/P&L/linkage in CR-1472. Add remaining cross-currency cash-leg variants and cash-in-lieu fractional overlay behavior. | Existing transaction contract packs and all remaining combined parity paths pass. |
...
| Historical AVCO lot evidence | partial | CR-1479 establishes current aggregate state on deterministic rebuild and keeps new ordered source rows reconciled. CR-1480 adds idempotent, bounded, replay-derived audit/apply with row-count/value certification and PostgreSQL rollback proof. Execute it against the production-like estate before treating tax-lot source products as current after cutover. | Reviewed dry-run/apply reports show no failed/drifted keys and source-product supportability evidence passes. |
| Cost-history runtime complexity | partial | CR-1466 removes the FX N+1 pattern; CR-1467 removes FIFO availability scans; CR-1468 makes AVCO source allocation linear; CR-1471 removes full-history replay from strictly ordered safe appends; CR-1473 bounds FIFO restoration; CR-1479 bounds AVCO application restoration. At 8,000 rows, FIFO and AVCO disposal each restore 1 source at 0.065ms and 0.086ms respectively; backdated rebuild remains a complete 8,001-row operation. | Query-count/explain/end-to-end load evidence meets target without weakening deterministic replay; measure set-based AVCO source reconciliation and lock behavior under deployed-like volume. |
| Throughput and capacity | partial | CR-1471 adds deterministic engine-only opening/disposal/backdated mode evidence and bounded mode/restore metrics. Measure database and Kafka end-to-end events/second, p50/p95/p99, pool utilization, query count, lag, failure recovery, and shutdown drain against the three-service baseline. | No material regression; bounded in-flight and per-portfolio ordering proven. |
