...
| Backdated cost correction persistence | partial | CR-1463 proves the defect and atomically persists the deterministic affected suffix while publishing only the incoming event. FIFO and AVCO later-disposal corrections, rollback, position epoch, and event count pass. Add direct backdated multi-lot fee and cross-currency cases. | Backdated multi-lot/FX/fee tests prove current and later transaction costs, realized P&L, outbox events, duplicate replay, and rollback behavior; no stale future cost rows remain. |
