...
| `POST /reporting/holdings-snapshot/query` | Operational Read | No active direct internal downstream consumer evidenced in current local repo truth | Deprecated convenience shape | Holdings snapshot query retained as a migration bridge. Target product is `HoldingsAsOf`; remove/keep decision now tracked in `lotus-core #312` and still depends on merged or external consumer confirmation. |
