...
## Findings
- `scripts/quality/test_manifest.py` had already removed suite aliases, but active RFCs, conformance reports, and the `Makefile` still referenced deleted names such as `buy-rfc`, `sell-rfc`, and `fx-rfc`.
