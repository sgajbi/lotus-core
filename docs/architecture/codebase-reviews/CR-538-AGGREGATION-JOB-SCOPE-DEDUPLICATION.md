# CR-538: Aggregation Job Scope Deduplication

Date: 2026-05-31

## Scope

Query-service operations support repository aggregation-job count and list queries.

## Finding

`OperationsRepository.get_aggregation_jobs_count(...)` and
`OperationsRepository.get_aggregation_jobs(...)` both maintained the same support scope:

1. portfolio ownership,
2. as-of support evidence guard,
3. governed stored job status predicate,
4. aggregation business date,
5. job id,
6. correlation id.

Duplicating that scope made aggregation operations pagination vulnerable to future count/list drift
when adding or correcting operational filters.

## Change

1. Added `_apply_aggregation_job_scope(...)` as the shared aggregation-job filter helper.
2. Reused that helper from both aggregation-job count and list queries.
...
is a maintainability hardening slice that keeps aggregation operations pagination aligned to one
