...
2. `make database-hot-path-evidence` runs five exact PostgreSQL test nodes and assembles six
   scenarios: latest positions, operations support paging, reconciliation controls, transaction
   ledger count/page, and valuation-job claim.
...
4. Read statements use `EXPLAIN (ANALYZE, BUFFERS, FORMAT JSON)`. The mutating valuation claim is
   executed and explained in separate savepoints that are both rolled back; evidence proves the
   durable queue remains unchanged.
5. Fragment assembly rejects missing, extra, malformed, misidentified, or unsafe evidence. The
...
The exact clean-head command at signed SHA `dd27adb1f0939723599439ff6ff34e7a4ffa889c`
completed five PostgreSQL nodes in 91.45 seconds and produced six results:
...
| valuation job claim | failed | 1,000 | 20,001 | sequential scan of `portfolio_valuation_jobs`; routed to #985 |

The retained content identity was
`sha256:91a45293c2d74295e292b1198104a5b98af625a129952c397210d43dda4e82e5`.
...
- Warning-strict database-evidence unit suite: 25 passed.
- Exact clean-head repository command: 5 PostgreSQL nodes passed in 91.45 seconds; six fragments
  assembled with four passed and two failed report-only scenarios.
- Prior focused PostgreSQL proofs passed for ledger (74.10 seconds), reconciliation (65.49
  seconds), operations support (77.21 seconds), and rollback-safe valuation claim (67.23 seconds).
...
The ledger-page scan is durably routed to #506; the valuation-claim scan is a distinct, deduplicated
finding owned by #985. #509 remains closed because identity-first page hydration and bounded output
are intact. #503 retains broader reconciliation streaming, while #794/#795 retain outbox and Kafka
capacity. No runtime defect is hidden or silently repaired inside the report framework.
