...
8. Make the `--skip-cleanup` reuse path an explicit authority upgrade: update the portfolio master
   ...
   already-completed readiness stage. Repeat the terminal-failure check after authority becomes
   durable so an in-flight job that fails during the upgrade cannot escape the initial snapshot or
   enter downstream seed continuation.
...
- `tests/unit/tools/test_front_office_portfolio_seed.py`: `103 passed` after rebasing onto main
