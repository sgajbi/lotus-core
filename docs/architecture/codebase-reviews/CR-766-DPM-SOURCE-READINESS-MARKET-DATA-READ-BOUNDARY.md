# CR-766 DPM Source Readiness Market Data Read Boundary
...
DPM source-readiness orchestration still wrapped the downstream market-data coverage source read
inline, including the fail-closed unavailable-source policy.

That kept market-data source read semantics in the broad integration service instead of the DPM
readiness module that owns source-family availability policy and evaluated-universe scope.

## Action

Added `dpm_source_market_data_read_or_none(...)` to `dpm_source_readiness.py`, then routed the
service through that helper before resolving the market-data source-family outcome.

The service still owns source call ordering and the concrete market-data coverage dependency. The
DPM readiness module now owns reusable market-data evaluated-universe passthrough and fail-closed
unavailable-source suppression. Focused helper coverage locks evaluated-universe passthrough, empty
scope passthrough, and unavailable-source suppression.
