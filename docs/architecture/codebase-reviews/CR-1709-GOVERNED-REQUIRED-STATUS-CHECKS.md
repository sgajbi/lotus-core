...
2. `required_status_checks_guard.py` validates the closed manifest shape, expands matrix suites,
   ...
   includes, declarations inside GNU Make column-zero-scoped `define` bodies, `$(eval)`/`${eval}` outside recipes, parser/execution-variable assignments, special execution targets, `vpath`, dynamic or continued declarations, inherited process state, parse-time/recipe output,
...
declarations inside GNU Make column-zero-scoped `define` bodies, `$(eval)`/`${eval}` outside recipes, parser/execution-variable assignments, special execution targets, `vpath`, dynamic/continued declarations, inherited `MAKELEVEL`/`CI`/runner state, parse-time/recipe output,
