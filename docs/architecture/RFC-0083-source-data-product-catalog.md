...
| `PortfolioMaturitySummary` | Operational read | `query_service` | gateway, report, manage, advise, idea | `/portfolios/{portfolio_id}/maturity-summary` |
...
`ExternalHedgeExecutionReadiness:v1` have active
fail-closed runtime routes that return `UNAVAILABLE` until external treasury ingestion is
certified. Real ingested treasury evidence still requires bank-owned ingestion, persistence,
supportability tests, and live evidence before downstream workflows may treat the products as
available:

| Product | Current posture | Intended source evidence | Explicit non-claim |
| --- | --- | --- | --- |
| `ExternalCurrencyExposure:v1` | Active fail-closed route: `/integration/portfolios/{portfolio_id}/external-currency-exposure`. | Bank-supplied currency exposure evidence by portfolio, mandate, currency, and as-of date. | No local FX attribution, hedge advice, treasury instruction, execution readiness, or execution claim. |
| `ExternalHedgePolicy:v1` | Active fail-closed route: `/integration/portfolios/{portfolio_id}/external-hedge-policy`. | Bank-supplied hedge policy constraints, allowed hedge posture, and policy lineage. | No hedge-policy approval, hedge advice, counterparty choice, treasury instruction, order generation, best execution, or OMS acknowledgement. |
| `ExternalFXForwardCurve:v1` | Active fail-closed route: `/integration/market-data/external-fx-forward-curve`. | Bank-supplied FX forward curve observations with source refs and freshness posture. | No local forward pricing, FX valuation methodology, hedge advice, treasury instruction, counterparty selection, order generation, best execution, venue routing, OMS acknowledgement, fills, settlement, or autonomous treasury action. |
| `ExternalEligibleHedgeInstrument:v1` | Active fail-closed route: `/integration/portfolios/{portfolio_id}/external-eligible-hedge-instruments`. | Bank-supplied eligible hedge instrument reference evidence by portfolio, mandate, exposure currency, instrument type, and as-of date. | No eligible-instrument selection, suitability approval, product recommendation, counterparty selection, treasury instruction, best execution, or OMS acknowledgement. |
...
## External OMS Source Product

RFC42-WTBD execution-boundary coverage now has a governed source-product boundary for
bank-owned OMS acknowledgement data. `ExternalOrderExecutionAcknowledgement:v1` has an active
fail-closed runtime route at
`/integration/portfolios/{portfolio_id}/external-order-execution-acknowledgement` that returns
`UNAVAILABLE` until external OMS acknowledgement ingestion is certified. Real ingested OMS evidence
still requires bank-owned ingestion, persistence, supportability tests, and live evidence before
downstream workflows may treat the product as available:

| Product | Current posture | Intended source evidence | Explicit non-claim |
| --- | --- | --- | --- |
| `ExternalOrderExecutionAcknowledgement:v1` | Active fail-closed route: `/integration/portfolios/{portfolio_id}/external-order-execution-acknowledgement`. | Bank-supplied order execution acknowledgement evidence by portfolio, mandate, execution intent, order reference, and as-of date. | No order generation, venue routing, best execution, OMS acknowledgement, fills, settlement, execution-status certification, or autonomous execution action. |
...
| --- | --- | --- | --- |
| `HoldingsAsOf:v1` | Portfolio positions, source-owned instrument maturity dates when Core reference data carries them, cash-balance totals, source-reported cash weight, portfolio/base currency, as-of date, supportability metadata, latest evidence timestamp, deterministic cash-balance evidence fingerprint, cash-balance snapshot identity, current-epoch snapshot reconciliation, history-backed supplement posture, and cash reporting-currency restatement. The implementation-backed methodology is documented in `docs/methodologies/source-data-products/holdings-as-of.md`. | Consumers may use returned cash totals, source-reported cash weight, source identity, source-owned maturity dates, and holdings rows as source facts with the product metadata attached. `lotus-idea` must consume the source-reported cash weight, maturity dates, and source identity rather than reconstructing Core-owned cash/AUM or instrument lifecycle facts locally. | Consumers must not infer liquidity ladders, income need, performance returns, risk exposure methodology, tax advice, execution quality, reinvestment recommendation, cash-deployment recommendation, or OMS acknowledgement from holdings, cash totals, source-owned maturity dates, or source-reported cash weight alone. |
...
| `DpmPortfolioUniverseCandidate:v1` | Paged Core-owned DPM portfolio-universe candidates from effective discretionary mandate bindings rooted in the tenant admitted at the request boundary, with explicit selection-basis, as-of, booking-center, model-portfolio, active-authority, continuation-token, lineage, and supportability evidence. | Consumers may use the returned candidates as source-owned cohort discovery inputs before campaign or wave composition, preserving the `EFFECTIVE_DISCRETIONARY_MANDATE_BINDING` basis. | Consumers must not infer relationship-householding, suitability approval, PM ranking, execution readiness, client communication workflow, or external workflow ownership from candidate membership alone. |

`DiscretionaryMandateBinding:v1` is the source-owned mandate profile surface for first-wave DPM.
In addition to portfolio, client, model, policy, jurisdiction, risk, and rebalance binding, it now
publishes mandate objective, review cadence, last review date, and next review due date when the
mandate administration source provides them. Missing objective or review schedule fields degrade the
product supportability state instead of allowing downstream systems to treat local defaults as
source truth.
| `TransactionLedgerWindow:v1` | Deterministically ordered booked transaction rows, trade fees, transaction-cost records, withholding tax, other interest deductions, net interest, realized capital/FX/total P&L fields, linked cashflow records, FX/event linkage identifiers, and optional field-aware reporting-currency restatements, including explicit row-level realized FX P&L local evidence restated from trade currency when present. The implementation-backed methodology is documented in `docs/methodologies/source-data-products/transaction-ledger-window.md`. | Consumers may preserve explicit row-level measures, lineage, supportability posture, source field, source unit, selected row identity, and optional `realized_fx_pnl_local_reporting_currency` for reporting surfaces. | Consumers must not aggregate rows into tax methodology, FX attribution, cash movement methodology, transaction-cost methodology, or execution-quality conclusions unless a source owner publishes that methodology. |
| `PortfolioCashflowProjection:v1` | Daily booked/projected/net points and totals in portfolio currency, bounded to 366 days, with admitted-tenant metadata, a common comparable `source_cut_id`, source-cut materialization time, source-row and component-total controls, explicit empty-window supportability, deterministic request/snapshot/content identity, and separate input/calculation/output SHA-256 lineage. The implementation-backed methodology is documented in `docs/methodologies/source-data-products/portfolio-cashflow-projection.md`. | Consumers may use points and totals only when admitted scope, common cut, digest, reconciliation, supportability, and calculation lineage are accepted together. A supported empty window is explicit zero-source evidence, not a fabricated event timestamp. | Consumers must reject blocked or mismatched trust evidence and must not treat the projection as a liquidity ladder, funding recommendation, income plan, OMS execution forecast, market-impact estimate, or client cash-need methodology. |
| `PortfolioCashMovementSummary:v1` | Latest cashflow rows grouped by classification, timing, currency, and flow scope, with admitted-tenant metadata, a common comparable `source_cut_id`, source-cut materialization time, SQL row-count and per-currency total controls, explicit empty-window supportability, deterministic request/snapshot/content identity, and separate input/calculation/output SHA-256 lineage. The implementation-backed methodology is documented in `docs/methodologies/source-data-products/portfolio-cash-movement-summary.md`. | Consumers may use bucket totals, counts, directions, and evidence time only when admitted scope, common cut, digest, reconciliation, supportability, and calculation lineage are accepted together; unlike currencies remain separate. | Consumers must reject blocked or mismatched trust evidence and must not treat the summary as a forecast, funding recommendation, treasury instruction, liquidity advice, execution-quality assessment, tax methodology, or OMS acknowledgement. |
| `PortfolioMaturitySummary:v1` | Core-owned booked contractual maturity posture over exact `HoldingsAsOf:v1` instrument scopes. The receipt binds tenant/correlation metadata, deterministic holdings snapshot/content/source-batch/policy identity, exact portfolio-day/epoch reconciliation, the requested maturity window, next maturity date, maturing holding count, supportability reasons, and separate input/calculation/output SHA-256 lineage. The implementation-backed methodology is documented in `docs/methodologies/source-data-products/portfolio-maturity-summary.md`. | Consumers such as `lotus-idea` may use a current, `SUPPORTED`, scope-matched, digest-consistent receipt as source evidence for upcoming maturity review posture without scanning raw holdings rows or reconstructing maturity schedules locally. | Consumers must reject missing, incomplete, stale, failed, or unknown reconciliation and projected state. They must not treat this summary as a full callable/putable/amortizing/structured maturity schedule, liquidity advice, reinvestment recommendation, performance or risk methodology, tax advice, execution-quality evidence, or OMS acknowledgement. |
...
| `PortfolioRealizedTaxSummary:v1` | Portfolio-level aggregation of explicit booked withholding-tax and other-interest-deduction evidence by ledger currency, with optional reporting-currency restatement. The implementation-backed methodology is documented in `docs/methodologies/source-data-products/portfolio-realized-tax-summary.md`. | Consumers may use currency totals, evidence-row counts, source-window counts, runtime supportability, and latest evidence timestamp as source-owned realized-tax evidence for outcome review and proof packs. | Consumers must not claim tax advice, after-tax optimization, tax-loss harvesting suitability, client-tax approval, jurisdiction-specific recommendations, tax-reporting certification, execution quality, or OMS acknowledgement from this summary. |
| `ClientTaxProfile:v1` | Effective-dated client tax-reference profile facts including tax residency, booking tax jurisdiction, bounded tax status, source-supplied withholding rate, tax-applicability flags, treaty codes, eligible account types, supportability metadata, lineage, and source record identity. The implementation-backed methodology is documented in `docs/methodologies/source-data-products/client-tax-profile.md`. | Consumers may use the returned profile rows as source-owned tax-reference evidence for DPM governance and proof packs with supportability and lineage attached. | Consumers must not claim tax advice, after-tax optimization, tax-loss harvesting suitability, client-tax approval, jurisdiction-specific recommendations, tax-reporting certification, or OMS acknowledgement from profile facts. |
| `ClientTaxRuleSet:v1` | Effective-dated client tax-rule references including tax year, jurisdiction, rule code/category/status/source, asset/security/income applicability, source-supplied rate and threshold fields, supportability metadata, lineage, and source record identity. The implementation-backed methodology is documented in `docs/methodologies/source-data-products/client-tax-rule-set.md`. | Consumers may use the returned rule rows as source-owned tax-rule reference evidence for DPM governance and proof packs with supportability and lineage attached. | Consumers must not claim tax advice, tax-loss harvesting suitability, after-tax optimization, client-tax approval, jurisdiction-specific recommendations, tax-reporting certification, best execution, or OMS acknowledgement from rule references. |
...
| `ExternalCurrencyExposure:v1` | Fail-closed external treasury exposure posture for currency-overlay workflows. It resolves portfolio mandate identity and returns `UNAVAILABLE` with missing treasury data families, blocked capabilities, lineage, runtime metadata, and deterministic fingerprints until bank-owned treasury ingestion is certified. | Consumers may use the unavailable posture to block currency-overlay realization and route operators to treasury-source integration work. | Consumers must not claim FX attribution, hedge advice, treasury instructions, execution readiness, OMS acknowledgement, fills, settlement, or autonomous treasury action from this posture. |
| `ExternalHedgePolicy:v1` | Fail-closed external treasury hedge-policy posture for currency-overlay workflows. It resolves portfolio mandate identity and returns `UNAVAILABLE` with missing external hedge-policy data, blocked capabilities, lineage, runtime metadata, empty policy rules, and deterministic fingerprints until bank-owned treasury policy ingestion is certified. | Consumers may use the unavailable posture to block hedge-policy realization and route operators to treasury-source integration work. | Consumers must not claim hedge-policy approval, hedge advice, treasury instructions, counterparty selection, order generation, best execution, OMS acknowledgement, fills, settlement, or autonomous treasury action from this posture. |
| `ExternalEligibleHedgeInstrument:v1` | Fail-closed external treasury eligible-instrument posture for currency-overlay workflows. It resolves portfolio mandate identity and returns `UNAVAILABLE` with missing external eligible-instrument data, blocked capabilities, lineage, runtime metadata, empty eligible instruments, deterministic fingerprints, and implementation-backed methodology in `docs/methodologies/source-data-products/external-eligible-hedge-instrument.md` until bank-owned treasury instrument eligibility ingestion is certified. | Consumers may use the unavailable posture to block eligible-instrument-dependent currency-overlay realization and route operators to treasury-source integration work. | Consumers must not claim eligible-instrument selection, hedge-instrument suitability, product recommendation, treasury instructions, counterparty selection, order generation, best execution, OMS acknowledgement, fills, settlement, or autonomous treasury action from this posture. |
| `ExternalFXForwardCurve:v1` | Fail-closed external treasury FX forward curve posture for currency-overlay workflows. It returns `UNAVAILABLE` with requested currency pairs and tenors echoed for audit, empty curve points, missing external curve data, blocked capabilities, lineage, runtime metadata, deterministic fingerprints, and implementation-backed methodology in `docs/methodologies/source-data-products/external-fx-forward-curve.md` until bank-owned treasury curve ingestion is certified. | Consumers may use the unavailable posture to block forward-curve-dependent currency-overlay realization and route operators to treasury-source integration work. | Consumers must not claim forward pricing, FX valuation methodology, hedge advice, treasury instructions, counterparty selection, order generation, best execution, venue routing, OMS acknowledgement, fills, settlement, or autonomous treasury action from this posture. |
| `ExternalHedgeExecutionReadiness:v1` | Fail-closed external treasury readiness posture for currency-overlay workflows. It resolves portfolio mandate identity and returns `UNAVAILABLE` with required external treasury data families, blocked capabilities, lineage, runtime metadata, deterministic fingerprints, and implementation-backed methodology in `docs/methodologies/source-data-products/external-hedge-execution-readiness.md` until bank-owned treasury ingestion is certified. | Consumers may use the unavailable posture to block currency-overlay realization and route operators to treasury-source integration work. | Consumers must not claim hedge advice, forward pricing, counterparty selection, best execution, OMS acknowledgement, fills, settlement, or autonomous treasury action from this posture. |
| `ExternalOrderExecutionAcknowledgement:v1` | Fail-closed external OMS acknowledgement posture for DPM execution workflows. It resolves portfolio mandate identity and returns `UNAVAILABLE` with missing external OMS acknowledgement data, blocked capabilities, lineage, runtime metadata, empty acknowledgements, deterministic fingerprints, and implementation-backed methodology in `docs/methodologies/source-data-products/external-order-execution-acknowledgement.md` until bank-owned OMS ingestion is certified. | Consumers may use the unavailable posture to block execution-acknowledgement realization and route operators to OMS source-integration work. | Consumers must not claim order generation, venue routing, best execution, OMS acknowledgement, fills, settlement, execution-status certification, or autonomous execution action from this posture. |
...
| `PerformanceComponentEconomics:v1` | Row-level transaction, cashflow, fee, tax, income, realized P&L, and FX-context economics evidence with grouped component totals and coverage metadata. The implementation-backed methodology is documented in `docs/methodologies/source-data-products/performance-component-economics.md`. | `lotus-performance` may use the returned rows, totals, source refs, runtime metadata, and component-family coverage to replace locally inferred component economics in contribution analytics sourcing. | Consumers must not treat this product as contribution analytics, attribution analytics, performance returns, tax advice, execution-quality evidence, best-execution evidence, venue routing, OMS acknowledgement, or UI/demo readiness by itself. |

These boundaries are intentionally conservative. `lotus-core` is the source authority for recorded
portfolio, transaction, cashflow, tax-lot, client tax-reference, and observed-fee facts; it is not the owner of
...
populates optional reporting-currency fields from latest available FX rates using field-aware source
currency selection for book versus trade/local fields, including
...
`PortfolioRealizedTaxSummary:v1` now has implementation-backed methodology truth in
`docs/methodologies/source-data-products/portfolio-realized-tax-summary.md`. The method filters
booked transaction rows by portfolio, date window, and effective as-of date; includes only rows with
explicit `withholding_tax_amount` or `other_interest_deductions_amount` evidence; aggregates
withholding, other deductions, total tax amount, and evidence-row counts by ledger currency; can
optionally restate grouped totals into a requested reporting currency from latest available FX
rates; and preserves the non-claim boundary from tax advice, after-tax optimization, tax-loss
harvesting suitability, client-tax approval, jurisdiction-specific recommendations,
tax-reporting certification, execution quality, and OMS acknowledgement.
...
`ExternalOrderExecutionAcknowledgement:v1` now has implementation-backed methodology truth in
`docs/methodologies/source-data-products/external-order-execution-acknowledgement.md`. The method
resolves active discretionary mandate identity, echoes optional execution intent and order
references for audit, emits empty acknowledgements with `UNAVAILABLE` supportability, marks
`external_oms_order_execution_acknowledgement` missing, publishes blocked capabilities, and emits
fail-closed lineage plus deterministic runtime fingerprints without claiming order generation,
venue routing, best execution, OMS acknowledgement, fills, settlement, execution-status
certification, or autonomous execution action.
...
cash totals, cash-weight supportability, reporting-currency scope, and latest evidence timestamp,
and expose `snapshot_id` as `holdings_as_of_cash_balances:<fingerprint>` for downstream lineage
and replay evidence.
...
For snapshot-backed cross-currency positions, market-data evidence includes the exact price date
and the persisted `valuation_fx_rate_date` plus `valuation_fx_rate` used to calculate the stored
value. The FX pair is database-enforced as jointly null or jointly present with a positive finite
rate, and the exact value is bound into market-data identity so a same-date FX correction changes
lineage. A current price status alone does not certify FX freshness. Legacy or new cross-currency
rows without both parts of that lineage, and rows whose FX date precedes the requested date, are
unavailable until revaluation produces exact-date evidence. Same-currency valuation has no FX
input and therefore retains null FX evidence.
