...
| `PortfolioCashMovementSummary:v1` | Portfolio-level aggregation of latest cashflow rows by classification, timing, currency, and flow scope. The implementation-backed methodology is documented in `docs/methodologies/source-data-products/portfolio-cash-movement-summary.md`. | Consumers may use bucket totals, row counts, movement direction, latest evidence timestamp, source fingerprint, and data-quality posture as source-owned cash movement evidence. | Consumers must not treat the summary as a forecast, funding recommendation, treasury instruction, liquidity advice, execution-quality assessment, tax methodology, or OMS acknowledgement. |
