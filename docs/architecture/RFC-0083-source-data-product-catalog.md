...
| `PortfolioMaturitySummary:v1` | Core-owned contractual maturity posture over `HoldingsAsOf:v1` instrument maturity facts, including the requested maturity window, next maturity date, maturing holding count, freshness, supportability reasons, and deterministic request fingerprint. The implementation-backed methodology is documented in `docs/methodologies/source-data-products/portfolio-maturity-summary.md`. | Consumers such as `lotus-idea` may use the summary as source evidence for upcoming maturity review posture without scanning raw holdings rows or reconstructing maturity schedules locally. | Consumers must not treat this summary as a full callable/putable/amortizing/structured maturity schedule, liquidity advice, reinvestment recommendation, performance or risk methodology, tax advice, execution-quality evidence, or OMS acknowledgement. |
