...
`ExternalCurrencyExposure:v1` and `ExternalHedgeExecutionReadiness:v1` have active fail-closed
runtime routes that return `UNAVAILABLE` until external treasury ingestion is certified. The
remaining products stay `proposed` in the repo-native domain-product declaration until ingestion,
persistence, APIs, supportability tests, and live evidence are implemented:
