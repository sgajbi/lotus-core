...
| `PortfolioCashflowProjection:v1` | Daily booked/projected/net points and totals in portfolio currency, bounded to 366 days, with tenant/correlation metadata, source-row and component-total controls, explicit empty-window supportability, deterministic request/snapshot/content identity, and separate input/calculation/output SHA-256 lineage. The implementation-backed methodology is documented in `docs/methodologies/source-data-products/portfolio-cashflow-projection.md`. | Consumers may use points and totals only when the scope, digest, reconciliation, supportability, and calculation lineage are accepted together. A supported empty window is explicit zero-source evidence, not a fabricated timestamp. | Consumers must reject blocked or mismatched trust evidence and must not treat the projection as a liquidity ladder, funding recommendation, income plan, OMS execution forecast, market-impact estimate, or client cash-need methodology. |
| `PortfolioCashMovementSummary:v1` | Latest cashflow rows grouped by classification, timing, currency, and flow scope, with tenant/correlation metadata, SQL row-count and per-currency total controls, explicit empty-window supportability, deterministic request/snapshot/content identity, and separate input/calculation/output SHA-256 lineage. The implementation-backed methodology is documented in `docs/methodologies/source-data-products/portfolio-cash-movement-summary.md`. | Consumers may use bucket totals, counts, directions, and evidence time only when the scope, digest, reconciliation, supportability, and calculation lineage are accepted together; unlike currencies remain separate. | Consumers must reject blocked or mismatched trust evidence and must not treat the summary as a forecast, funding recommendation, treasury instruction, liquidity advice, execution-quality assessment, tax methodology, or OMS acknowledgement. |
