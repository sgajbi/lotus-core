...
| `ExternalFXForwardCurve:v1` | Fail-closed external treasury FX forward curve posture for currency-overlay workflows. It returns `UNAVAILABLE` with requested currency pairs and tenors echoed for audit, empty curve points, missing external curve data, blocked capabilities, lineage, runtime metadata, and deterministic fingerprints until bank-owned treasury curve ingestion is certified. | Consumers may use the unavailable posture to block forward-curve-dependent currency-overlay realization and route operators to treasury-source integration work. | Consumers must not claim forward pricing, FX valuation methodology, hedge advice, treasury instructions, counterparty selection, order generation, best execution, venue routing, OMS acknowledgement, fills, settlement, or autonomous treasury action from this posture. |
