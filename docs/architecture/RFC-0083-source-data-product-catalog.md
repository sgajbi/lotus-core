...
| --- | --- | --- | --- |
| `HoldingsAsOf:v1` | Portfolio positions, cash-balance totals, source-reported cash weight, portfolio/base currency, as-of date, supportability metadata, latest evidence timestamp, deterministic cash-balance evidence fingerprint, cash-balance snapshot identity, current-epoch snapshot reconciliation, history-backed supplement posture, and cash reporting-currency restatement. The implementation-backed methodology is documented in `docs/methodologies/source-data-products/holdings-as-of.md`. | Consumers may use returned cash totals, source-reported cash weight, source identity, and holdings rows as source facts with the product metadata attached. `lotus-idea` must consume the source-reported cash weight and cash-balance source identity rather than reconstructing Core-owned cash/AUM facts locally. | Consumers must not infer liquidity ladders, income need, performance returns, risk exposure methodology, tax advice, execution quality, cash-deployment recommendation, or OMS acknowledgement from holdings, cash totals, or source-reported cash weight alone. |
