...
| `lotus-gateway #124` | Closed on April 17, 2026. Gateway commit `d63d8e5` correctly removed `as_of_date` shaping while the Core overview contract was unbounded. Core issue #856 subsequently added an optional governed `as_of_date`: bounded callers receive aggregation-health evidence through that date, while callers that omit it retain the full operator queue view. Readiness continues to own date-scoped domain validation. | Keep closed. A downstream consumer may adopt the bounded overview parameter only when its workflow intentionally needs historical aggregation-health evidence; operator support views should omit it to preserve full-queue visibility. |
...
| Route | Active downstream consumers verified | Integration posture |
| --- | --- | --- |
| `POST /integration/portfolios/{portfolio_id}/core-snapshot` | `lotus-gateway`, `lotus-risk` | Gateway and risk are correct direct consumers and remain aligned to the governed top-level `PortfolioStateSnapshot` envelope. `lotus-idea` is now an approved catalog-intended consumer for opportunity-intelligence sourcing, but this audit does not evidence a live direct `lotus-idea` client binding yet. Merged report PR `#41` removed the remaining report-side `core-snapshot` dependency, migrated review composition onto strategic summary/allocation/positions/transactions routes, and passed the remote PR Merge Gate, so report is no longer a live direct consumer of this route. No active direct `lotus-manage` or `lotus-advise` client was found in this pass. |
...
| --- | --- | --- | --- | --- |
| `PortfolioStateSnapshot` | `POST /integration/portfolios/{portfolio_id}/core-snapshot` | `lotus-gateway`, `lotus-risk`, catalog-intended `lotus-manage`, `lotus-advise`, `lotus-report`, `lotus-idea` | Direct active client evidence exists in `lotus-gateway/src/app/clients/lotus_core_query_client.py` and `lotus-risk/src/app/integrations/lotus_core_client.py`. Merged report PR `#41` removed the remaining direct `core-snapshot` client seam and migrated report review composition onto strategic routes with a green remote PR Merge Gate, so report is now catalog-intended rather than a live direct consumer. `lotus-manage` currently documents intended adoption but does not yet have an active outbound client. `lotus-advise` currently uses other lotus-core seams more directly than `core-snapshot` itself. `lotus-idea` is approved for governed opportunity-intelligence sourcing but still needs direct client and runtime proof before it is described as a live consumer. | Strong for gateway and risk. `lotus-report`, `lotus-manage`, `lotus-advise`, and `lotus-idea` should not be described as live direct consumers of this route until their product workflows bind to it. |
...
| `BenchmarkAssignment` | `POST /integration/portfolios/{portfolio_id}/benchmark-assignment` | `lotus-performance`, `lotus-gateway`, `lotus-manage`, catalog-intended `lotus-idea` | `lotus-performance` returns and benchmark services; `lotus-gateway/src/app/clients/lotus_core_query_client.py` for workspace composition context. `lotus-manage/src/infrastructure/core_sourcing/client.py` has a bounded resolver path used by mandate-health refresh to preserve benchmark identity lineage. `lotus-idea` is approved for governed opportunity-intelligence context but still needs direct client and runtime proof before it is described as a live consumer. `lotus-risk` and `lotus-report` remain catalog-intended rather than live direct consumers in this pass. | Strong for performance, gateway, and bounded Manage benchmark-identity consumption. Manage and Idea must not treat benchmark assignment as benchmark analytics, benchmark composition, active-risk, performance attribution, or model-approval methodology. |
...
| `PerformanceComponentEconomics` | `POST /integration/portfolios/{portfolio_id}/performance-component-economics` | `lotus-performance` | Issue `sgajbi/lotus-core#424` introduced the dedicated performance component economics source product so `lotus-performance` can consume core-authored cashflow, fee, income, tax, realized P&L, and FX-context evidence instead of reconstructing component economics locally. Current core evidence covers route/OpenAPI/catalog/security/service/repository/domain-product tests and the endpoint sources from `transactions`, `transaction_costs`, and `cashflows`. Direct downstream code evidence remains tracked in `sgajbi/lotus-performance#250`. | Strong for core producer certification. This is source-owned economics evidence, not contribution analytics, attribution analytics, performance returns, tax advice, execution-quality evidence, best-execution evidence, venue routing, OMS acknowledgement, or UI/demo readiness. Downstream `lotus-performance` integration proof is required before contribution source economics may move from `SOURCE_LIMITED` to `SOURCE_BACKED`. |
...
| `ExternalCurrencyExposure` | `POST /integration/portfolios/{portfolio_id}/external-currency-exposure` | `lotus-manage` | RFC39-WTBD-008 source-owner runtime posture introduces a fail-closed external treasury exposure product so `lotus-manage` can block currency-overlay realization when bank-owned treasury ingestion is not certified. Current core evidence covers route/OpenAPI/catalog/security/service/domain-product tests and explicit `UNAVAILABLE` supportability with missing external treasury data families, empty exposure rows, and blocked capabilities. Direct downstream code evidence must be added in `lotus-manage` before any manage proof pack may advertise source-backed currency-overlay exposure evidence. | Strong for core producer certification. This is source-owner supportability only, not FX attribution, hedge advice, treasury instruction, execution readiness, OMS acknowledgement, fills, settlement, or autonomous treasury action. Downstream `lotus-manage` integration and live canonical proof are required before RFC39-WTBD-008 can move beyond partial. |
| `ExternalHedgePolicy` | `POST /integration/portfolios/{portfolio_id}/external-hedge-policy` | `lotus-manage` | RFC39-WTBD-008 source-owner runtime posture introduces a fail-closed external treasury hedge-policy product so `lotus-manage` can block currency-overlay hedge-policy realization when bank-owned treasury policy ingestion is not certified. Current core evidence covers route/OpenAPI/catalog/security/service/domain-product tests and explicit `UNAVAILABLE` supportability with missing hedge-policy data, empty policy rules, and blocked capabilities. Direct downstream code evidence must be added in `lotus-manage` before any manage proof pack may advertise source-backed hedge-policy evidence. | Strong for core producer certification. This is source-owner supportability only, not hedge-policy approval, hedge advice, treasury instruction, counterparty selection, order generation, best execution, OMS acknowledgement, fills, settlement, or autonomous treasury action. Downstream `lotus-manage` integration and live canonical proof are required before RFC39-WTBD-008 can move beyond partial. |
| `ExternalEligibleHedgeInstrument` | `POST /integration/portfolios/{portfolio_id}/external-eligible-hedge-instruments` | `lotus-manage` | RFC39-WTBD-008 source-owner runtime posture introduces a fail-closed external treasury eligible-instrument product so `lotus-manage` can block eligible-instrument-dependent currency-overlay realization when bank-owned treasury instrument eligibility ingestion is not certified. Current core evidence covers route/OpenAPI/catalog/security/service/domain-product tests and explicit `UNAVAILABLE` supportability with missing eligible-instrument data, empty eligible instruments, requested exposure currencies and instrument types echoed for audit, and blocked capabilities. Direct downstream code evidence must be added in `lotus-manage` before any manage proof pack may advertise source-backed eligible hedge instrument evidence. | Strong for core producer certification. This is source-owner supportability only, not eligible-instrument selection, hedge-instrument suitability, product recommendation, treasury instruction, counterparty selection, order generation, best execution, OMS acknowledgement, fills, settlement, or autonomous treasury action. Downstream `lotus-manage` integration and live canonical proof are required before RFC39-WTBD-008 can move beyond partial. |
| `ExternalFXForwardCurve` | `POST /integration/market-data/external-fx-forward-curve` | `lotus-manage` | RFC39-WTBD-008 source-owner runtime posture introduces a fail-closed external treasury FX forward curve product so `lotus-manage` can block forward-curve-dependent currency-overlay realization when bank-owned treasury curve ingestion is not certified. Current core evidence covers route/OpenAPI/catalog/security/service/domain-product tests and explicit `UNAVAILABLE` supportability with missing curve data, empty curve points, requested currency pairs and tenors echoed for audit, and blocked capabilities. Direct downstream code evidence must be added in `lotus-manage` before any manage proof pack may advertise source-backed forward-curve evidence. | Strong for core producer certification. This is source-owner supportability only, not forward pricing, FX valuation methodology, hedge advice, treasury instruction, counterparty selection, order generation, best execution, venue routing, OMS acknowledgement, fills, settlement, or autonomous treasury action. Downstream `lotus-manage` integration and live canonical proof are required before RFC39-WTBD-008 can move beyond partial. |
...
| `ExternalOrderExecutionAcknowledgement` | `POST /integration/portfolios/{portfolio_id}/external-order-execution-acknowledgement` | `lotus-manage`, `lotus-gateway` | RFC42-WTBD execution-boundary source-owner runtime posture introduces a fail-closed external OMS acknowledgement product so `lotus-manage` can block execution-acknowledgement realization when bank-owned OMS acknowledgement ingestion is not certified. Current core evidence covers route/OpenAPI/catalog/security/service/domain-product tests and explicit `UNAVAILABLE` supportability with missing external OMS acknowledgement data, empty acknowledgement rows, requested execution intent and order references echoed for audit, and blocked capabilities. Direct downstream code evidence exists in `lotus-gateway/src/app/clients/lotus_core_query_client.py` for the bounded supportability lookup; Gateway preserves the fail-closed posture and does not claim OMS execution. Direct downstream code evidence must be added in `lotus-manage` before any manage proof pack may advertise source-backed OMS acknowledgement evidence. | Strong for core producer certification and bounded Gateway supportability lookup. This is source-owner supportability only, not order generation, venue routing, best execution, OMS acknowledgement, fills, settlement, execution-status certification, or autonomous execution action. Downstream `lotus-manage` integration and live canonical proof are required before execution acknowledgement can move beyond partial. |
...
| `PortfolioManagerBookMembership` | `POST /integration/portfolio-manager-books/{portfolio_manager_id}/memberships` | Catalog-approved `lotus-manage`, `lotus-gateway` | RFC41-WTBD-001 source-owner foundation introduced the PM-book membership source product so `lotus-manage` can discover explicit wave cohorts without fabricating PM books locally. CR-1633 resolves effective accepted portfolio-manager roles first and permits `portfolios.advisor_id` only for portfolios with no role-assignment history. Gateway #500 provides the implemented caller-context-bounded advisor-book facade for Workbench; #513 owns the coordinated consumer-contract and canonical proof closure. | Strong for Core producer certification and implemented Gateway consumption. Gateway's repo-native RFC-0084 consumer declaration, federated discovery evidence, and controlled Core/Gateway/Workbench live proof have been completed for #513. Manage's automatic PM-book wave discovery still requires separate downstream and live proof. This is not staff hierarchy, relationship householding, entitlement, advisory authority, or legal ownership. |
| `PortfolioPartyRoleAssignment` | `POST /integration/portfolios/{portfolio_id}/party-role-assignments` | Catalog-approved `lotus-gateway`, `lotus-workbench`, `lotus-manage` | #513 introduces effective-dated, source-versioned relationship coverage, investment-advisory, portfolio-management, delegate, and client-service truth. The Core producer has model, migration, ingestion, as-of query, OpenAPI, catalog, security, and compatibility tests. | Strong for Core producer proof. Gateway #500 and Workbench #450 completed their downstream adoption obligations for #513; broad party ownership remains separately governed by #518. Consumers must not invent team, delegate, supervisor, household, or entitlement truth. |
...
| `DpmPortfolioUniverseCandidate` | `POST /integration/dpm/portfolio-universe/candidates` | `lotus-manage` | RFC37-WTBD-004 source-owner depth now has a bounded Core producer contract for paged DPM portfolio-universe candidate discovery. The endpoint sources from effective discretionary mandate bindings and preserves as-of, booking-center, model-portfolio, active-authority, continuation-token, supportability, snapshot, and lineage metadata. | Strong for core producer certification once route/OpenAPI/catalog/security/service/domain-product gates pass. Downstream `lotus-manage` consumption and live canonical wave proof remain separate work before this can be claimed as production-proven end-to-end. This is not relationship-householding, suitability approval, PM ranking, execution readiness, client communication workflow, or external workflow ownership. |
| `BenchmarkDefinition` | `POST /integration/benchmarks/{benchmark_id}/definition`; `POST /integration/benchmarks/catalog` | Catalog-approved `lotus-performance`, `lotus-risk`, `lotus-gateway` | Core publishes effective benchmark master, constituent, version, source-lineage, and supportability evidence from the QCP-owned reference-data boundary. `lotus-gateway` directly uses the catalog route for workspace benchmark-selection and discovery flows before a targeted benchmark identifier is known. | Strong for Core route, OpenAPI, catalog, service, repository, and package-independence proof, plus bounded Gateway catalog usage. This audit does not claim the catalog-approved performance or risk consumers are active direct callers until their client/runtime evidence is revalidated. |
...
| `BenchmarkReturnSeriesWindow` | `POST /integration/benchmarks/{benchmark_id}/return-series` | Catalog-approved `lotus-performance`, `lotus-risk` | Core publishes the bounded benchmark return window with deterministic source lineage and temporal semantics. | Strong for Core publication and contract tests. Direct performance/risk adoption remains a downstream proof obligation and is not inferred from catalog binding alone. |
| `IndexDefinition` | `POST /integration/indices/catalog` | Catalog-approved `lotus-performance`, `lotus-risk` | Core publishes effective index identity and source metadata through the QCP-owned index catalog use case. | Strong for Core route, catalog, OpenAPI, and package ownership. Live direct-caller claims remain intentionally narrower than the approved consumer binding. |
