...
| `ExternalOrderExecutionAcknowledgement` | `POST /integration/portfolios/{portfolio_id}/external-order-execution-acknowledgement` | `lotus-manage` | RFC42-WTBD execution-boundary source-owner runtime posture introduces a fail-closed external OMS acknowledgement product so `lotus-manage` can block execution-acknowledgement realization when bank-owned OMS acknowledgement ingestion is not certified. Current core evidence covers route/OpenAPI/catalog/security/service/domain-product tests and explicit `UNAVAILABLE` supportability with missing external OMS acknowledgement data, empty acknowledgement rows, requested execution intent and order references echoed for audit, and blocked capabilities. Direct downstream code evidence must be added in `lotus-manage` before any manage proof pack may advertise source-backed OMS acknowledgement evidence. | Strong for core producer certification. This is source-owner supportability only, not order generation, venue routing, best execution, OMS acknowledgement, fills, settlement, execution-status certification, or autonomous execution action. Downstream `lotus-manage` integration and live canonical proof are required before execution acknowledgement can move beyond partial. |
...
| `BenchmarkDefinition` | `POST /integration/benchmarks/{benchmark_id}/definition`; `POST /integration/benchmarks/catalog` | Catalog-approved `lotus-performance`, `lotus-risk` | Core publishes effective benchmark master, constituent, version, source-lineage, and supportability evidence from the QCP-owned reference-data boundary. | Strong for Core route, OpenAPI, catalog, service, repository, and package-independence proof. This audit does not claim both consumers are active direct callers until their client/runtime evidence is revalidated. |
