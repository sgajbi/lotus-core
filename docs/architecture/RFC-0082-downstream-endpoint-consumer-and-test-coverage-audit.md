...
| `BenchmarkAssignment` | `POST /integration/portfolios/{portfolio_id}/benchmark-assignment` | `lotus-performance`, `lotus-gateway`, `lotus-manage` | `lotus-performance` returns and benchmark services; `lotus-gateway/src/app/clients/lotus_core_query_client.py` for workspace composition context. `lotus-manage/src/infrastructure/core_sourcing/client.py` has a bounded resolver path used by mandate-health refresh to preserve benchmark identity lineage. `lotus-risk` and `lotus-report` remain catalog-intended rather than live direct consumers in this pass. | Strong for performance, gateway, and bounded Manage benchmark-identity consumption. Manage must not treat benchmark assignment as benchmark analytics, benchmark composition, active-risk, performance attribution, or model-approval methodology. |
