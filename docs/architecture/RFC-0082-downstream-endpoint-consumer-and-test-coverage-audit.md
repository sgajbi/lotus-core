...
| `PortfolioManagerBookMembership` | `POST /integration/portfolio-manager-books/{portfolio_manager_id}/memberships` | `lotus-manage` | RFC41-WTBD-001 source-owner foundation introduced the PM-book membership source product so `lotus-manage` can discover explicit wave cohorts without fabricating PM books locally. CR-1633 now resolves effective accepted portfolio-manager roles first and permits `portfolios.advisor_id` only for portfolios with no role-assignment history. | Strong for core producer certification. Downstream `lotus-manage` integration and live canonical wave proof are required before automatic PM-book wave discovery is production-proven. This is not staff hierarchy, relationship householding, entitlement, advisory authority, or legal ownership. |
