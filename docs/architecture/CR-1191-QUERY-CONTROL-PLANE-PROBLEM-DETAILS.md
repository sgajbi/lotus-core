...
- Custom QCP 422 response examples now use a shared dual-media helper that documents both the
  application-level `application/problem+json` error and FastAPI's default `application/json`
  request-validation payload for malformed bodies.
