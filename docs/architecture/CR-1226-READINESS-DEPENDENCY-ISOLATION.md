...
- Added `DependencyReadinessResult` and explicit readiness statuses:
  `ok`, `unavailable`, `timeout`, `misconfigured`, and `error`.
- Added dependency configuration pre-checks before expensive probes run; empty Kafka bootstrap
  configuration is classified as `misconfigured` instead of attempting a broker metadata call.
...
- Preserved the existing top-level response shape:
  ...
  - not-ready responses still raise HTTP 503 with `detail.status="not_ready"` and the dependency
    map.
- Added tests for DB timeout, Kafka timeout, Kafka misconfiguration, dependency exception
  isolation, cached ready, cached not-ready, unknown dependencies, and mixed dependency states.
...
Operators can distinguish dependency `timeout`, `unavailable`, `misconfigured`, and unexpected
`error` states while clients retain the existing top-level ready/not-ready contract.

## Tests Added

- DB timeout classification.
- Kafka timeout classification.
- Kafka misconfiguration classification.
...
contract expansion is that dependency status values can now include `timeout`, `misconfigured`, and
`error` in addition to the existing `ok` and `unavailable` values.
