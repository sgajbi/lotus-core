...
open current-epoch holding in the same portfolio and resolved as-of date. The response now also
publishes:

- `source_reported_cash_weight_denominator_portfolio_currency`,
- `source_reported_cash_weight_supportability`.

If denominator evidence is incomplete, missing, contains null market values, zero or negative, or
stale relative to the resolved as-of date, the weight and denominator return null with an explicit
blocked supportability posture. The route does not invent liquidity advice, idle-cash
recommendations, deployment actions, or downstream analytics claims.
...
- deterministic tests for populated, incomplete/missing, zero, stale, and precision behavior.
