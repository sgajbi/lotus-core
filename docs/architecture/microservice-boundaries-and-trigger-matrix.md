...
| `portfolio_transaction_processing_service` | Atomic booked-transaction cost, cashflow, position, idempotency, compatibility outbox, and replay-request processing. | Delivery mappers, `ProcessTransactionUseCase`, separate financial modules/policies, ports, SQLAlchemy adapters, one unit of work, two consumers, aggregate health/metrics. | Transaction processing runtime. | Typed commands/results, financial module ports, replay port, observer port, compatibility event contracts. | Active app-local/CI deployable; registry/Kubernetes cutover pending. |
...
| `pipeline_orchestrator_service` | Scales with atomic transaction-completion and portfolio control events. | Orchestration rules can change independently of transaction modules. | Stage-gate operations owner is distinct from transaction-processing owners. | Owns pipeline stage state. | Stage-gate failures should be visible without corrupting transaction state. | Internal orchestration boundary; no public API exposure. |
...
| Transaction stage gate | `pipeline_orchestrator_service` | authoritative atomic completion on `transactions.cost.processed` | `transaction_processing.ready`, `portfolio_security_day.valuation.ready` | `(stage_name, transaction_id, epoch)` ownership; late/duplicate completion facts cannot emit twice. | Pipeline stage state and readiness evidence. |
...
| `portfolio_transaction_processing_service` | Atomic transaction cost, cashflow, and position runtime with separate module authority | `transaction_costs`, `position_lot_state`, `average_cost_pool_state`, `cashflows`, `position_history`, `daily_position_snapshots`, `position_state`, idempotency/outbox | `transactions.persisted`, `transactions.reprocessing.requested` | `transactions.cost.processed`, `cashflows.calculated` compatibility events plus position state | Event |
| `pipeline_orchestrator_service` | Stage-gate orchestrator for deterministic downstream readiness | `pipeline_stage_state` | `transactions.cost.processed`, `portfolio_day.aggregation.completed`, `portfolio_day.reconciliation.completed` | `transaction_processing.ready`, `portfolio_security_day.valuation.ready`, `portfolio_day.reconciliation.requested`, `portfolio_day.controls.evaluated` | Event |
...
3. The target emits `transactions.cost.processed` as its authoritative atomic completion fact and
   retains `cashflows.calculated` as a compatibility fact with no active in-repo consumer.
4. `pipeline_orchestrator_service` consumes the authoritative completion fact and emits
   `transaction_processing.ready` plus `portfolio_security_day.valuation.ready`; it does not wait
   for a separately delivered cashflow event, and the target position module does not consume the
   readiness output.
