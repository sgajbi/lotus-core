...
| `pipeline_orchestrator_service` | Portfolio-day reconciliation and control-stage coordination plus quiescence. | Portfolio-day application handlers, control-stage repository, event consumers, and event mapping. | Portfolio-day controls coordinator. | Control-stage repository contract, event-mapping adapters, quiescence support contracts. | Current-state retirement candidate after remaining portfolio-day transitions are reassigned under #712. |
...
| `pipeline_orchestrator_service` | Scales only with portfolio aggregation/reconciliation control events after transaction readiness retirement. | Portfolio-day control coordination can change independently of transaction modules. | Financial-control operations ownership remains distinct from transaction processing. | Owns surviving `FINANCIAL_RECONCILIATION` control-stage rows. | Control coordination failures do not corrupt transaction state or transaction readiness. | Internal orchestration boundary; no public API exposure. |
...
| `pipeline_orchestrator_service` | Portfolio-day reconciliation and control-stage coordinator | surviving `FINANCIAL_RECONCILIATION` rows in `pipeline_stage_state` | `portfolio_day.aggregation.completed`, `portfolio_day.reconciliation.completed` | `portfolio_day.reconciliation.requested`, `portfolio_day.controls.evaluated` | Event |
...
4. The target also emits `transactions.cost.processed` and `cashflows.calculated` as compatibility
   facts with no active in-repo consumers. `pipeline_orchestrator_service` is not on the transaction
   readiness path.
