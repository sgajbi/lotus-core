...
Kubernetes manifests remain pending and must never run target and legacy topologies together.
...
3. The target emits `transactions.cost.processed` and `cashflows.calculated` compatibility events.
4. `pipeline_orchestrator_service` retains the compatibility stage wait and emits
   `transaction_processing.ready` plus `portfolio_security_day.valuation.ready`; the target position
   module does not wait for or consume that normal-path signal.
