...
| `portfolio_derived_state_service` | Position-timeseries materialization, durable portfolio aggregation scheduling, portfolio-timeseries materialization, and completion/reconciliation staging. | Separate position and portfolio application/domain modules, typed ports, service-owned adapters, one supervisor, and one outbox dispatcher. | Portfolio derived-state capability. | Position repository contracts, aggregation scheduler ports, market-data port, completion event contract. | Consolidated current-state deployable; retain independent workload metrics and concurrency. |
...
| `portfolio_derived_state_service` | Position-event consumption scales by preserved Kafka group; portfolio fan-in scales through independently configured bounded DB workers. | Position and portfolio modules can change independently inside one release. | One Core derived-state owner with explicit module boundaries. | Owns position/portfolio timeseries and durable aggregation jobs through separate adapters. | One module failure fails the supervised process; durable offsets, jobs, leases, idempotent upserts, and outbox state support restart without partial publication. | Headless worker with health/readiness/metrics/version only. |
...
| `portfolio_derived_state_service` | Position-timeseries event handling plus lease-aware portfolio aggregation | `position_timeseries`, `portfolio_timeseries`, `portfolio_aggregation_jobs` | `valuation.snapshot.persisted`, durable `portfolio_aggregation_jobs` | `portfolio_day.aggregation.completed`, `portfolio_day.reconciliation.requested` | Event + durable queue + bounded workers |
