...
rows. It joins `transaction_costs` and `cashflows` through existing ORM relationships.
...
Rows are ordered by `security_id`, `transaction_date`, and `transaction_id`.
...
| --- | --- |
| `cashflow` | linked `cashflows.amount`, `currency`, `classification`, `timing`, flow-scope flags |
| `fee` | explicit `transaction_costs.amount` rows, falling back to `transactions.trade_fee` |
...
`component_totals` groups non-zero component amounts by `component_family` and currency. Tax totals
combine withholding tax and other interest deductions in the same currency while preserving row-level
fields separately.
