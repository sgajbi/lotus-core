...
| Consumer Lag | Gauge | `service`, `topic`, `group_id`, `partition` | Alert on the three subscriptions that drive position effects: `transactions.persisted` (`portfolio_transaction_processing_group`), `transactions.reprocessing.requested` (`portfolio_transaction_replay_request_group`), and — for governed corporate actions — `corporate_action.manifest.received` (`corporate_action_manifest_group`). A stall on the manifest group blocks corporate-action position effects while the other two look healthy. Do **not** monitor `transactions.cost.processed`; nothing subscribes to it, so it cannot reveal a stuck worker. |
