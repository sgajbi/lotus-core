...
| **Producer Failing to Publish** | `202 Accepted` responses, but no data appears downstream. | `kafka.producer.delivery_failed` / `Kafka message delivery failed.` in this service's logs, alongside the ingestion job's own failure state. This service publishes directly through `KafkaEventPublisher` and does **not** use an outbox, so `outbox_events_*` metrics belong to other services and stay flat during an ingestion outage. `kafka_publish_errors_total` is never updated at all. | **Investigate Kafka health.** The cluster may be degraded (e.g., disk full, leader election issues). |
