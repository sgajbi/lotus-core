...
| **Producer Failing to Publish** | `202 Accepted` responses, but no data appears downstream. | `Kafka message delivery failed.` or `OutboxDispatcher: Kafka delivery failed; will retry later.` in service logs. Watch `outbox_events_pending` and `outbox_events_oldest_pending_age_seconds` rising, and `outbox_events_failed_total` incrementing. `kafka_publish_errors_total` is **never updated** — nothing calls the helper that increments it, so it stays at zero during an outage. | **Investigate Kafka health.** The cluster may be degraded (e.g., disk full, leader election issues). |
