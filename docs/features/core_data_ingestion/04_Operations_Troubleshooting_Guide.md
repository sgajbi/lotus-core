...
| **Publish Rejected Synchronously** | `503 Service Unavailable` with a `Retry-After` header, not a `202`. | The publish raised `IngestionPublishError`, the router called `raise_ingestion_publish_unavailable()`, and the ingestion job is marked **failed**. Look for the failed job by its `job_id`, which is included in the response detail. | **Investigate Kafka health.** The cluster may be degraded (e.g., disk full, leader election issues). The caller should retry after the advertised interval. |
