...
| **Delivery Fails After Acceptance** | `202 Accepted` was already returned, but no data appears downstream. | `kafka.producer.delivery_failed` / `Kafka message delivery failed.` in this service's logs. On paths that do not call `_confirm_delivery`, the callback arrives after the response, so the request stays accepted and its ingestion job stays **queued** — do **not** search for a failed job here, there will not be one. This service publishes directly through `KafkaEventPublisher` and does **not** use an outbox, so `outbox_events_*` metrics belong to other services and stay flat during an ingestion outage, and `kafka_publish_errors_total` is never updated at all. | **Investigate Kafka health**, then reconcile from the queued job rather than expecting a failure state. |
