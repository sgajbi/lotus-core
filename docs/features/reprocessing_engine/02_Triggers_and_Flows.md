...
3.  **Event Replay:** The service queries the database for **all** historical transactions for that key. It then re-publishes them in chronological order to the `transactions.cost.processed` topic.
...
1.  **Detection (`PriceEventConsumer`):** A consumer listens for `market_prices.persisted` events. It compares the `price_date` to the system's latest `business_date`. If the price is for a past date, it's flagged.
...
3.  **Durable Conversion (`ValuationScheduler`):** On its polling cycle, the scheduler transaction asks `InstrumentReprocessingConversionRepository` to claim ordered triggers with `FOR UPDATE SKIP LOCKED`, delete each claimed row, and stage the matching `RESET_WATERMARKS` job before commit. A staging failure rolls the deletion back. A concurrent earlier price update is retained and coalesced into the pending job on a subsequent conversion.
4.  **Watermark Reset:** The reprocessing worker claims the durable job and performs the fan-out operation:
   ...
   One `PENDING` reset job per security retains the earliest known date. A job already in `PROCESSING` is never mutated; a newly arriving earlier date creates a separate pending generation.
