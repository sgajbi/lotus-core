...
its source-currency evidence retains a zero-ending position only when same-day valuation evidence
matches the latest position quantity in the active epoch; a same-day position cash-flow can
qualify only alongside that executable valuation row and in the same epoch. Other reporting
endpoints continue to apply their own snapshot predicates. This keeps each query bounded while
preserving true historical as-of semantics.
