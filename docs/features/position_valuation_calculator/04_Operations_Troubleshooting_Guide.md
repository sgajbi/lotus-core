...
| :--- | :--- | :--- | :--- | :--- |
| **`position_state_watermark_lag_days`** | **Gauge** | none | `valuation_orchestrator_service` | **Last-sampled value only — not a progress signal.** `ValuationBackfillPlanner._observe_backfill_gap_metrics()` calls `.set()` once per iterated state inside the backfill loop, and the gauge carries no labels, so it holds whichever key was sampled last. When no keys need backfill the loop does not run and the previous value persists unchanged — so it goes stale precisely when the backlog has drained. Use `scheduler_gap_days` for aggregate distribution and the support APIs for per-key truth. |
...
* **Price arrives before any open position key** *(orchestrator)*: the handling splits on the price date, and so does the evidence.
  * **Current-date price** — `decide_source_revaluation_schedule()` returns `stage_durable_replay=False`. `PriceEventConsumer._stage_reprocessing_if_needed()` logs `"Current price has no visible positions; later position readiness will use the persisted price."` and returns **without** staging anything. There is deliberately **no** `instrument_reprocessing_state` row; later position readiness picks up the persisted price. Grep that message; do not look for a durable trigger.
  * **Back-dated or future-dated price** — `stage_durable_replay` is true, a durable replay trigger is staged, and an `instrument_reprocessing_state` row for the security is the evidence.
* **Watermark advance** *(orchestrator)*: no stable log literal to grep for, and **no metric that confirms it either**. `position_state_watermark_lag_days` is an unlabelled gauge overwritten per key, so a falling value may only mean the last-sampled key was fresher, and a flat value may mean the loop stopped running. Confirm advance through the support APIs and the durable per-key watermark in `position_state`. `"ValuationScheduler normalized no-history states to current watermark."` covers only the no-history case.
...
| :--- | :--- | :--- | :--- |
| **Positions Not Valued** | Data in the query APIs is stale and `scheduler_gap_days` shows an elevated backlog distribution. Do not rely on `position_state_watermark_lag_days` here — it reports the last key sampled and holds its previous value when the backfill loop finds nothing to do. | `GET /support/portfolios/{portfolio_id}/valuation-jobs?status_filter=PENDING` shows growing pending jobs. | **Cause:** `ValuationScheduler` in `valuation_orchestrator_service` may not be dispatching jobs, or this service's `ValuationConsumer` workers may be stalled. <br> **Resolution:** Check the orchestrator's scheduler logs and this service's consumer logs. Correlate with `GET /support/portfolios/{portfolio_id}/overview`. |
...
| **Quiet-Day Timeseries Row Missing After Price Ingestion** | A position exists on day N and day N+1 market prices are present, but analytics only returns the cash leg or an incomplete security set for day N+1. | Check the price date first, because the two cases have different evidence. For a **back-dated or future** price, look for missing or stale rows in `instrument_reprocessing_state` / support lineage views. For a **current-date** price, no such row is ever created — grep `Current price has no visible positions; later position readiness will use the persisted price.` in `valuation_orchestrator_service` instead. | **Cause:** A price arrived before the open position key was visible. For a back-dated price, the durable replay trigger did not get drained. For a current-date price, position readiness never consumed the persisted price. <br> **Resolution:** Back-dated — verify `instrument_reprocessing_state` and `reprocessing_jobs` are clearing, then check scheduler watermark reset fan-out. Current-date — check `ValuationReadinessConsumer` is progressing, since it is what turns the persisted price into work. |
