...
| Equity corporate actions | Split, reverse split, consolidation, bonus issue, and stock dividend | `limited` | Same-instrument actions restate FIFO and AVCO original/open lot quantities by one exact ratio while conserving local/base basis. The whole event is rejected when any source lot cannot be represented at governed quantity precision; no remainder is rounded or silently allocated. Lot/position divergence is rejected before commit and is discoverable with `make audit-lot-position-parity`. Parent-event persistence and complete lot-lineage receipts remain under [#480](https://github.com/sgajbi/lotus-core/issues/480) and [#481](https://github.com/sgajbi/lotus-core/issues/481). |
