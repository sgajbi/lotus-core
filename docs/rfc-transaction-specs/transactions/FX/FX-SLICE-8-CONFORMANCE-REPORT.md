...
| --- | --- |
| Canonical FX business types | `src/libs/portfolio-common/portfolio_common/domain/transaction/type_registry.py`; `tests/unit/transaction_specs/test_fx_slice0_characterization.py` |
| Canonical validation and reason codes | `src/services/portfolio_transaction_processing_service/app/domain/transaction/fx/models.py`; `src/services/portfolio_transaction_processing_service/app/domain/transaction/fx/validation.py`; `src/services/portfolio_transaction_processing_service/app/domain/transaction/fx/reason_codes.py`; `tests/unit/services/portfolio_transaction_processing_service/domain/transaction/fx/test_validation.py` |
| Deterministic linkage and enrichment | `src/services/portfolio_transaction_processing_service/app/domain/transaction/fx/linkage.py`; `tests/unit/services/portfolio_transaction_processing_service/domain/transaction/fx/test_linkage.py` |
| Cash settlement semantics | `src/services/portfolio_transaction_processing_service/app/domain/cashflow/calculation.py`; `tests/integration/services/portfolio_transaction_processing_service/test_cashflow_rule_contract.py`; `tests/unit/services/portfolio_transaction_processing_service/domain/cashflow/test_calculation.py` |
| FX contract lifecycle | `src/services/portfolio_transaction_processing_service/app/domain/transaction/fx/contract_instrument.py`; `src/services/portfolio_transaction_processing_service/app/application/cost_basis_processing/execution.py`; `src/services/portfolio_transaction_processing_service/app/domain/position/reducer.py`; `tests/unit/services/portfolio_transaction_processing_service/domain/transaction/fx/test_contract_instrument.py`; `tests/unit/services/portfolio_transaction_processing_service/application/cost_basis_processing/test_execution.py`; `tests/unit/services/portfolio_transaction_processing_service/domain/position/test_reducer.py` |
| Swap grouping | `src/services/portfolio_transaction_processing_service/app/domain/transaction/fx/linkage.py`; `src/services/portfolio_transaction_processing_service/app/domain/transaction/fx/validation.py`; `tests/unit/services/portfolio_transaction_processing_service/domain/transaction/fx/test_linkage.py` |
...
1. `python -m pytest tests/unit/services/persistence_service/consumers/test_persistence_instrument_consumer.py tests/unit/services/portfolio_transaction_processing_service/application/cashflow_processing/test_use_case.py -q`
 - Result: `18 passed`
...
3. Embedded FX fees remain intentionally unsupported. Phase-1 uses separate linked `FEE`/`TAX`
   postings; future embedded support requires an explicit versioned fee-currency and charged-leg
   contract rather than inference.
