...
|---|---|---|---|---|
| 4. BUY Semantic Invariants | COVERED | MATCHES | `tests/unit/services/portfolio_transaction_processing_service/domain/cost_basis/calculation/test_cost_calculator.py`, `tests/unit/transaction_specs/test_buy_slice0_characterization.py` | BUY quantity/cost progression and explicit zero realized P&L locked in tests. |
| 5. BUY Processing Flow | PARTIALLY_COVERED | PARTIALLY_MATCHES | `src/services/portfolio_transaction_processing_service/app/application/cost_basis_processing/execution.py`, `tests/unit/services/portfolio_transaction_processing_service/application/cost_basis_processing/test_execution.py`, `tests/unit/services/portfolio_transaction_processing_service/domain/cashflow/test_calculation.py` | Lifecycle is implemented across ingestion/persistence/calculation/query with stage telemetry; a single persisted lifecycle-stage state machine is not yet implemented. |
...
| 7. BUY Validation Rules | COVERED | MATCHES | `tests/unit/services/portfolio_transaction_processing_service/domain/transaction/validation/test_trades.py`, `docs/rfc-transaction-specs/transactions/BUY/BUY-SLICE-1-VALIDATION-REASON-CODES.md` | Deterministic reason codes and strict metadata validation are implemented. |
| 8. BUY Calculation Rules and Formulas | COVERED | MATCHES | `tests/unit/services/portfolio_transaction_processing_service/domain/cost_basis/calculation/test_cost_calculator.py` | Calculation invariants and cross-currency rules are enforced. |
...
| 10. BUY Lot Rules | COVERED | MATCHES | `tests/integration/services/portfolio_transaction_processing_service/test_int_cost_repository_lot_offset.py` | Durable BUY lot-state creation/upsert behavior validated. |
| 11. BUY Cash and Dual-Accounting Rules | COVERED | MATCHES | `tests/unit/services/portfolio_transaction_processing_service/domain/cashflow/test_calculation.py`, `tests/unit/services/portfolio_transaction_processing_service/domain/cost_basis/calculation/test_cost_calculator.py` | BUY outflow sign, linkage propagation, and dual-currency handling validated. |
| 12. BUY Accrued-Interest Offset Rules | COVERED | MATCHES | `tests/integration/services/portfolio_transaction_processing_service/test_int_cost_repository_lot_offset.py` | Accrued-interest offset initialization and persistence are validated. |
| 13. BUY Timing Rules | PARTIALLY_COVERED | PARTIALLY_MATCHES | `tests/unit/services/portfolio_transaction_processing_service/domain/transaction/validation/test_trades.py` | Date ordering and settlement-date requirements are enforced; broader policy-level timing modes remain to be added with later transaction RFC rollout. |
...
| 17. BUY Test Matrix | COVERED | MATCHES | `scripts/quality/test_manifest.py` (`transaction-buy-contract` suite) | Dedicated regression suite now aggregates BUY-critical tests across unit/integration layers. |
| 19. BUY Configurable Policies | COVERED | MATCHES | `tests/unit/services/portfolio_transaction_processing_service/domain/cost_basis/calculation/test_cost_calculator.py` | BUY policy hook coverage exists for accrued-interest exclusion policy. |
...
- CI command: `python scripts/quality/test_manifest.py --suite transaction-buy-contract --with-coverage --coverage-file .coverage.transaction-buy-contract`
