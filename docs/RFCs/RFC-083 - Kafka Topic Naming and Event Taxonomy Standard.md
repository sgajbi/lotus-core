...
| `fixed_income.book_cost.authority.received` | governed fixed-income authority ingestion | `portfolio_transaction_processing_service` / `fixed_income_book_cost_authority_group` | exact source-lot authority fact | Active 12-partition source-lot ordered path. |
| `corporate_action.manifest.received` | source-owned corporate-action manifest ingestion | `portfolio_transaction_processing_service` / `corporate_action_manifest_group` | exact parent event, expected child graph, and source authority fact | Active 12-partition portfolio transaction-group ordered path. |
| `fixed_income.book_cost.disposal_replay.requested` | `portfolio_transaction_processing_service` authority UoW/outbox | `portfolio_transaction_processing_service` / `fixed_income_book_cost_correction_replay_group` | correction replay command | Active 12-partition source-lot command; republishes one earliest booked anchor with stable repair identity. |
