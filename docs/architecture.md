...
11. Ingestion business services and adapter-mode policy stay framework-neutral; FastAPI dependency
...
12. Bulk upload handling stays split across pure parsing/validation, commit orchestration, and a
   publisher adapter; see `docs/standards/bulk-upload-component-boundary-standard.md`.
13. Transaction replay stays split across pure replay planning, reader ports, and publisher
   adapters; see `docs/standards/transaction-replay-boundary-standard.md`.
14. Portfolio aggregation scheduler policy stays split across scheduler ports, infrastructure
   adapters, and pure dispatch planning; see
   `docs/standards/aggregation-scheduler-boundary-standard.md`.
15. Position calculation rules and backdated replay decisions stay pure and separate from
   persistence, outbox, metrics, and epoch-fencing orchestration; see
   `docs/standards/position-reducer-boundary-standard.md`.
16. Repositories own persistence access.
17. Shared cross-cutting behavior lives in `portfolio_common` or platform-owned standards where
   appropriate.
18. API contracts remain source-data-product aware, metadata-rich, and implementation-backed.
