...
| Documentation and CI measurement | The branch needed baseline measurement, incremental evidence, and a reviewable trail rather than cosmetic cleanup claims. | The quality scorecard, refactor report, baseline evidence, and CR-849 through CR-1037 ledger entries record measurable gate and maintainability movement; GitHub `Remote Feature Lane` passed for `828acc75` after CR-1036. | The final PR must refresh this row after the latest GitHub run on the final pushed commit is green. |
...
- Current measured source hotspot: `ingestion_job_service.py` `B (16.96)`;
  `reference_data_dto.py` is now `A (21.98)`, and the active non-generated C-ranked source
