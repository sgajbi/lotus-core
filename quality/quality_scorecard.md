...
| Complexity | Average `A (2.97)` with high-complexity hotspots still present; CR-880 reduced advisory proposal simulation from F to B, but Xenon enforcement remains report-only because the cost-calculator consumer still has an F-ranked block and `fx_linkage.py` remains a D-ranked module | Reduce remaining D/F hotspots by domain priority before promoting a broad complexity gate |
| Maintainability | No D/E/F source modules; enforced after CR-879 by `make quality-maintainability-gate`; existing C hotspots remain in OpenAPI enrichment and query-service repository/service modules | Keep maintainability regression-free while reducing existing C hotspots by domain priority |
