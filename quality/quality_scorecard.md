...
| Security | Bandit baseline clean and enforced after CR-875 by `make quality-bandit-gate` plus the quality-baseline Bandit security job; CR-1097 upgrades FastAPI, Starlette, python-multipart, and prometheus-fastapi-instrumentator runtime pins after Remote Feature Lane dependency audit found new CVEs, restoring `make security-audit` to no-known-vulnerability status | Keep Bandit and dependency-audit gates regression-free while avoiding cosmetic vulnerability ignores |
