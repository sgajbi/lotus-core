...
| Complexity | Average `A (2.97)` with high-complexity hotspots still present; CR-880 reduced advisory proposal simulation from F to B and CR-881 reduced the cost-calculator consumer from F to C. Xenon enforcement remains report-only because `fx_linkage.py` remains a D-ranked module | Reduce the remaining D-ranked hotspot before promoting a broad complexity gate |
