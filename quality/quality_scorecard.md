...
| CI governance | PR Merge Gate and Main Releasability no longer pin the Node 20-deprecated `actions/cache@v4`, `actions/upload-artifact@v4`, `actions/download-artifact@v4`, or `docker/setup-buildx-action@v3`; CR-1079 pins the current major action versions and adds a unit regression test; CR-1087 adds the missing `administration: read` permission to the required PR auto-merge workflow so branch-protection verification can read required status-check contexts before queueing merge | Keep action runtime warnings clean through workflow lint and pinned-version tests; verify the corrected auto-merge permission from `main` on the next PR if GitHub evaluates `pull_request_target` workflows from the pre-fix base revision for this PR |
...
| Area | Baseline / Risk Before This Branch | Current Evidence After CR-1087 | Remaining PR Risk |
| --- | --- | --- | --- |
