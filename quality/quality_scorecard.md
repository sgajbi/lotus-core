...
- CR-1263 adds `contracts/security/security-control-coverage.v1.json` and
  `make security-control-coverage-guard`, so every FastAPI app has static contract coverage for
  shared bootstrap, secure headers, CORS, metrics access, auth/audit, payload limits, upload limits
  where relevant, and safe error responses.
- CR-1265 adds a shared production security profile for service-local enterprise auth/audit
...
| Module size | CR-1714 enforces a 1,500-line cap over 1,144 tracked `src` Python modules; 9 existing oversized modules are banked at exact line counts with owner, rationale, issue, and expiry in `quality/module-size-baseline.v1.json` | Ratchet every reduction immediately and complete #462 until the baseline is empty |
...
| Source-batch lineage semantics | Some source-data products populated `source_batch_fingerprint` with request/snapshot fingerprints, conflating response identity with upstream batch lineage. | CR-1189 changes selected client tax, client restriction, sustainability preference, and DPM portfolio universe responses to leave `source_batch_fingerprint` null when true source-batch evidence is unavailable, while preserving `snapshot_id` and DPM `page.request_scope_fingerprint`. CR-1220 extends the correction to remaining active reference, liquidity, CIO, external treasury, and external OMS products found in this pass, and adds source-data contract guard coverage that rejects request/snapshot-scope expressions in `source_batch_fingerprint`. | Continue wiring persisted ingestion/source-batch evidence where available, and keep source-batch lineage regressions blocked through `make source-data-product-contract-guard`. |
| Async trace context | HTTP request handling produced trace context, but async event paths only propagated `correlation_id`, limiting incident traceability through outbox, Kafka consumers, DLQ, and replay. The HTTP fallback also used a fixed synthetic span id when generating `traceparent` from only a trace id. | CR-1247 adds shared `traceparent` normalization/context, preserves incoming HTTP `traceparent`, extends the governed event envelope with `traceparent`, and propagates it through outbox payloads, Kafka headers, shared consumers, DLQ publication, direct ingestion publish, and DLQ replay with focused unit and integration proof. CR-1248 rejects all-zero trace/span IDs and replaces the fixed HTTP fallback span id with a fresh non-zero W3C span id while preserving `X-Trace-Id` compatibility. | Add a future guard/inventory field for direct Kafka topic `traceparent` header support if direct publish coverage expands beyond current focused tests. Full OpenTelemetry span export, sampling policy, collector configuration, and APM integration remain separate follow-up work. |
...
| Transaction temporal vocabulary | The temporal vocabulary and RFC-0083 reconstruction model define current `transaction_date` as the transaction event/trade timestamp, but query-service transaction-date DTO descriptions used booking wording. | CR-1231 updates `TransactionRecord` and `SellDisposalRecord` DTO/OpenAPI semantics plus the wealth-reporting guide, keeps `booking_date` as a future target concept, regenerates the API vocabulary inventory, and adds OpenAPI regression assertions preventing transaction-date fields from drifting back to booking terminology. | Keep issue #440 open for PR/CI/QA evidence; future transaction booking work must introduce a first-class `booking_date` field and migration plan instead of overloading `transaction_date`. |
| Transaction type governance | Transaction-type semantics were spread across the cost enum, cashflow rules, position rules, query helpers, and RFC target docs, while `OTHER` was documented as a catch-all. | CR-1232 adds a canonical shared transaction type registry, classifies `OTHER` as migration-only, marks redemption/conversion target types as known but not implemented, and adds coverage tests over cost, cashflow, position-calculator, and query position-flow rule tables. CR-1233 fixes the first discovered drift by treating `SPIN_IN` and `DEMERGER_IN` as position transfer inflows and adds registry-to-position-rule conformance coverage. CR-1234 removes the cost-engine `OTHER` default-strategy bypass and requires production-booking cost enum values to have explicit strategies. CR-1235 derives query projected-position effect sets from registry `position_effect` semantics. CR-1236 derives cashflow transfer-sign sets from registry lifecycle and `position_effect` semantics while preserving the current `CASH_IN_LIEU` fallback. CR-1237 derives FX business validation/linkage sets from the registry and removes a duplicate local FX set. CR-1238 adds a reusable production lifecycle-family registry selector and derives portfolio-flow no-auto-generate guardrail sets from it. CR-1239 derives adjustment cash-leg auto-generation eligibility from registry-classified trade/income cash-leg semantics and conformance-guards resolver coverage. CR-1240 conformance-guards cost sorter cash dependency sets against registry compatibility while preserving cash-row trade-label inversion. CR-1241 derives e2e transaction coverage support sets from registry semantics so CI coverage follows canonical type classifications. | Continue issue #474 by replacing local hard-coded sets with registry-derived views and adding behavior conformance gates before enabling target redemption or conversion/exercise runtime support. |
...
| Simulation session unit of work | Simulation repository methods owned commits, rollbacks, UUID generation, clock selection, and version mutation, making user-facing simulation workflows hard to compose with audit, idempotency, replay, or lifecycle evidence. | CR-1243 makes `SimulationRepository` stage sessions, changes, status updates, and deletes only; `SimulationService` now owns deterministic clock/ID providers, expiry calculation, version increments, commit/refresh, and missing-delete rollback while preserving route and response behavior. | Keep future simulation audit, idempotency, replay, and lifecycle additions behind the application-service unit-of-work boundary rather than reintroducing repository-owned commits or infrastructure-owned providers. |
...
| Outbox failure evidence | Terminal outbox failures stored only status/count/timestamp fields while the concrete delivery reason remained log-only, weakening incident diagnosis and auditability. | CR-1187 adds nullable source-safe last-failure reason code, category, redacted bounded message, and timestamp fields; retryable and terminal delivery failures persist this metadata, and successful delivery clears stale failure evidence. CR-1221 adds protected QCP failed-row diagnostics through `GET /support/outbox/failed-events`, with filters and raw-payload exclusion. CR-1222 adds `outbox_recovery_audit` and governed `POST /support/outbox/failed-events/{outbox_id}/requeue` recovery with actor, reason, correlation, status transition, outcome evidence, and blind-requeue rejection. CR-1223 adds `GET /support/outbox/recovery-audits` so operators can review accepted and rejected recovery attempts without direct database access or raw payload exposure. CR-1224 adds bounded `outbox_recovery_attempts_total` metrics for accepted, rejected, not-found, and unexpected-error recovery attempts without high-cardinality operator identifiers. CR-1246 adds Docker-backed integrated proof that a dispatcher-created terminal failed row can be diagnosed through operations service, governed-requeued with audit evidence, and processed by the dispatcher afterward; it also adds `outbox_recovery_audit` to integration cleanup. | Decide alert-threshold values for recovery rejection/error outcomes after runtime use. |
...
| DLQ failure containment | DLQ publication failures previously left offsets uncommitted for safety, but sustained DLQ outages could redeliver the same terminal poison message indefinitely. | CR-1261 adds default-disabled `KAFKA_CONSUMER_DLQ_FAILURE_MAX_ATTEMPTS`; configured consumers now fail fast with source-safe `dlq_failure_budget_exhausted` logs/metrics after repeated DLQ failure for the same topic/group/partition/offset/key while preserving no-commit behavior. | Add durable service-owned quarantine only with an explicit retention, replay, and access-control design; do not claim local quarantine from the shared fail-fast budget alone. |
| Retryable consumer exhaustion | Retryable consumer failures intentionally left offsets uncommitted, but repeated retryable failures could loop indefinitely without terminal recovery. | CR-1262 adds default-disabled max-attempt and max-elapsed retryable failure budgets; exhausted retryable messages route through the existing DLQ path and commit only after DLQ publication succeeds. | Add cross-restart durable retry-attempt state only with a dedicated attempt store; current counters are in-process and terminal evidence becomes durable through DLQ. |
...
| Scheduler dispatch recovery | Valuation and aggregation scheduler claim-and-publish loops could leave jobs in `PROCESSING` after observed Kafka publish or delivery-confirmation failure, relying on later stale-job reset for recovery. | CR-1229 adds shared scheduler dispatch recovery vocabulary, repository-level immediate recovery for valuation and aggregation jobs, and scheduler hooks that requeue retryable claimed rows or fail exhausted rows according to max-attempt policy while preserving published versus unpublished record-key evidence. | Keep issue #596 open for PR/CI/QA evidence and Docker-backed scheduler proof; consider bounded recovery metrics if runtime use shows fleet-level visibility is needed beyond existing queue gauges and durable failure reasons. |
| Analytics export execution bounds | Inline analytics export creation marked jobs running and then collected/materialized results synchronously without a current-execution timeout or cancellation-state transition. | CR-1230 adds `LOTUS_CORE_ANALYTICS_EXPORT_EXECUTION_TIMEOUT_SECONDS`, bounds inline export collection/materialization with `asyncio.wait_for`, and records durable failed jobs with bounded timeout or cancellation reason text while preserving the existing response contract. | Keep issue #597 open for PR/CI/QA evidence and Docker-backed query-service runtime proof; treat a full async export runner as a separate lifecycle-mode change. |
| Ingestion publish dependency errors | Direct ingestion routers mapped Kafka publish dependency failures to generic HTTP 500 responses, while clients needed retryability, dependency identity, and partial-publish state to avoid unsafe blind retries. | CR-1218 adds a shared publish-error mapper and migrates direct ingestion publish routers to HTTP 503 with `Retry-After`, dependency=`kafka`, retryable metadata, correlation lineage, failed-record keys, publish state, and published-record count while preserving `INGESTION_PUBLISH_FAILED`, idempotency replay, and failure-history persistence. | Keep issue #594 open for PR/CI/QA evidence and decide separately whether event-replay/control-plane publish failures should use a sibling dependency-error mapper. |
| Post-publish bookkeeping repair | Direct ingestion post-publish and post-persist bookkeeping failures looked like generic failed work even though publish or persistence had already completed, leaving clients without retry-safety guidance and operators without a governed repair command. | CR-1228 expands `INGESTION_JOB_BOOKKEEPING_FAILED` with publish/work state, published count, `retry_safe=false`, recovery action/path, and supportability reason codes; adds shared bookkeeping recovery vocabulary; and adds protected `POST /ingestion/jobs/{job_id}/bookkeeping/repair` guarded by recorded bookkeeping-failure evidence. | Keep issue #595 open for PR/CI/QA evidence and Docker-backed proof against the real database and Kafka-backed ingestion stack; reuse evidence-gated repair for adjacent partial-failure workflows. |
| Ingestion retry recovery responses | Ingestion job retry mixed unsupported, duplicate, publish, and bookkeeping failure bodies, and retry publish failures could propagate raw downstream exceptions instead of a governed operator recovery response. | CR-1227 adds stable retry recovery details with `outcome`, `remediation`, and `recovery_path`, preserves existing `code` values and side effects, adds explicit `INGESTION_RETRY_PUBLISH_FAILED`, and keeps raw downstream publish/bookkeeping exception text out of the primary client message. | Keep issue #598 open for PR/CI/QA evidence and Docker-backed retry workflow proof; apply the same outcome/remediation pattern to adjacent recovery endpoints through issue-backed slices. |
...
| Cash-balance account-id provenance | Cash-balance fallback resolution could present a transaction `settlement_cash_account_id` string as a governed `cash_account_id` when cash-account master data was missing or stale, and raw transaction persistence did not classify unresolved settlement cash-account references at landing time. | CR-1200 validates transaction-derived fallback account mappings against active/effective `cash_account_masters`, adds additive `cash_account_id_source` provenance, includes the source in lineage fingerprints, and downgrades unresolved cash-security fallback rows to `PARTIAL`. CR-1256 adds `raw_transaction_cash_account_reference_policy_v1` so unresolved settlement cash-account references may land only as provisional raw evidence with structured supportability. | Keep issue #673 open only for PR/CI/QA and post-merge wiki publication evidence; evaluate stricter quarantine or database FK only after source-batch ordering and cash-account bounded-domain ownership are settled. |
| Instrument reference integrity | Product transaction and lot-state lifecycle rows could proceed through cost and BUY lot persistence without an instrument master row, leaving downstream products with normal-looking evidence for unresolved securities. | CR-1201 adds a cost-consumer guard that defers product transactions as retryable reference-data dependencies when instrument master data is missing, before cost-engine processing, transaction-cost persistence, BUY lot-state persistence, or processed-event publication. CR-1203 and CR-1204 add degraded read-side supportability for transaction-ledger and tax-lot source products with unresolved instrument references. CR-1255 adds an explicit raw transaction persistence policy: unresolved instruments may land only as provisional raw evidence for source-batch ordering tolerance, while lifecycle processing remains blocked. | Keep issue #674 open only for PR/CI/QA and post-merge wiki publication evidence; evaluate stricter source-batch quarantine or database FK only after ingestion ordering guarantees are strong enough. |
| Query-control-plane error contracts | Representative query-control-plane routers exposed downstream-facing failures as bare `detail` strings, often sourced from raw exception text, so clients lacked stable application error codes, correlation context, safe metadata, and consistent problem-details media-type contracts. | CR-1191 adds a shared problem-details contract and migrates representative core-snapshot, analytics-input, simulation, operations-support, and portfolio source-evidence failures to top-level problem-details payloads with stable QCP error codes, correlation IDs, bounded details, and safe metadata while preserving HTTP statuses. The shared response helper now documents migrated examples as `application/problem+json` and keeps unmigrated legacy bare-detail examples as explicit `application/json` contracts with a legacy schema. Source-evidence routes now expose `QCP_SOURCE_EVIDENCE_NOT_FOUND` and `QCP_SOURCE_EVIDENCE_INVALID_REQUEST` with source-product/portfolio metadata instead of raw service exception text. CR-1212 through CR-1216 extend the same pattern to selected integration source-data discovery, mandate-scoped source-route, benchmark reference, instrument enrichment, and benchmark market-series failures with stable QCP contracts and source-product metadata. CR-1217 adds `make qcp-problem-details-guard` to fail direct query-control-plane router `HTTPException` usage and raw `detail=str(...)` payloads. CR-1242 removes the remaining simulation-router substring classification by introducing typed simulation service errors and mapping exception types to QCP problem-details contracts. | Continue migrating any newly discovered control-plane route-family error-contract gaps and consider a later OpenAPI media-type inventory once false positives are understood. |
| Ingestion rate-limit enforcement scope | Ingestion write rate limiting was enabled by default but backed only by an in-process deque and local lock, so scaled workers or pods each received independent budgets while the operational control could be misread as global. | CR-1196 adds explicit `local_process`, `upstream_gateway`, and `local_process_with_upstream_gateway` scopes, requires a gateway policy ID for gateway-backed scopes at startup, logs the active enforcement contract, and emits bounded denial metrics/logs. CR-1225 adds `make ingestion-rate-limit-scope-guard` so implementation facts and docs continue to distinguish the default local-process guard from gateway-backed global enforcement claims. | Keep PR/CI/QA evidence attached to issue #684, validate the concrete platform gateway policy ID, and consider a Redis/shared-store token bucket if Lotus chooses service-owned global enforcement. |
...
| Boundary mapping conformance | Boundary mappings existed across ingestion, events, persistence, read records, and source-data envelopes, but there was no named conformance lane protecting representative mapping invariants. | CR-1184 adds `make test-boundary-mapping-conformance`, extracts transaction event-to-record values into an explicit mapper, and proves representative transaction and portfolio tax-lot source-data mappings without Kafka or a database. | Issue #665 is fixed locally pending PR CI/QA. Extend conformance coverage to more event families, API command/result mappers, typed read records, and source-data envelopes under umbrella issue #661. |
| Typed source-data read records | High-value source-data mappers accepted broad row objects, ORM tuple shapes, or ORM relationship objects, making persistence schema details part of application/source-data contracts. | CR-1257 adds `PortfolioTaxLotReadRecord` and maps `PortfolioTaxLotWindow:v1` repository rows to the typed record before source-data mapping and response assembly. CR-1258 adds `PerformanceEconomicsTransactionReadRecord`, `PerformanceEconomicsCashflowReadRecord`, and `PerformanceEconomicsCostReadRecord`, maps `PerformanceComponentEconomics:v1` repository output to those records, and extends boundary conformance to optional cashflow/cost evidence. | #664 is fixed locally pending PR/CI/QA. Continue applying this pattern to the remaining repository output-shape conversion backlog now guarded by CR-1259. |
| Repository output-shape guard | Legacy public repository methods still expose ORM return annotations, and before CR-1259 there was no static guard to prevent new drift while those surfaces are converted incrementally. | CR-1259 adds `docs/architecture/repository-output-shape-standard.md`, `make repository-output-shape-guard`, and a method-level transitional exception register that blocks new unregistered ORM returns and stale exceptions. | #648 is fixed locally pending PR/CI/QA. Continue shrinking the transitional exception register by converting remaining repository outputs to explicit records. |
| CI governance | PR Merge Gate and Main Releasability no longer pin the Node 20-deprecated `actions/cache@v4`, `actions/upload-artifact@v4`, `actions/download-artifact@v4`, or `docker/setup-buildx-action@v3`; CR-1079 pins the current major action versions and adds a unit regression test; CR-1103 moves the artifact family forward again after GitHub still emitted Node 20 deprecation annotations for artifact upload steps, requiring `actions/upload-artifact@v7` and `actions/download-artifact@v8` while retaining `actions/cache@v5` and `docker/setup-buildx-action@v4`; CR-1087 removes the branch-protection API probe from the required PR auto-merge workflow and adds a regression test so the default workflow token is not used for an unavailable Administration-scope endpoint; CR-1088 isolated `db_direct` suites from live workers; CR-1089 refines `integration-all` to use DB-plus-Kafka infrastructure without live application workers; CR-1090 explicitly waits for the `kafka-topic-creator` one-shot service before Kafka tooling tests run; CR-1093 waits for the `demo_data_loader` one-shot seed to complete before compose-backed latency measurements start, makes the Makefile latency seed-completion timeout explicit and overrideable for CI, makes app-local demo-data verification timing environment-driven with observable timeout state, and bounds PR/Main latency seed history to one year while preserving the three-year app-local default; CR-1159 makes absent `automerge` labels successful no-ops in PR auto-merge and adds a regression test so missing or removed labels do not emit stale skipped `Queue Auto Merge` check noise; CR-1160 aligns analytics latency windows to seeded business-day FX coverage while keeping real endpoint calls and p95 budgets; CR-1164 requires positive job timeouts across every workflow and restricts `continue-on-error` to governed report-only scope; CR-1165 promotes that guard into `make quality-workflow-governance-gate` and the quality-baseline workflow; CR-1169 routes collection gates through the test manifest and adds a named quality-baseline integration-lite collection job; CR-1179 adds a fast wiki docs gate for sidebar coverage, orphan pages, relative links, publication-safe names, optional publication parity, and quality-baseline enforcement; CR-1264 removes the app-validation non-blocking exception by checking out `lotus-platform`, setting `LOTUS_PLATFORM_ROOT`, and running `make lotus-core-validate` as a blocking PR Merge Gate job | Keep action runtime warnings clean through workflow lint, pinned-version tests, and GitHub CI log review; keep db-direct suites isolated from live-worker runtime and prove Docker-backed integration composition in GitHub CI; keep latency budgets unchanged while making the gate measure steady-state runtime; keep auto-merge queue signals actionable and low-noise; keep merge-critical CI fail-closed unless an exception is documented, tested, and governed |
...
| App-level validation evidence | CR-1107 adds `make lotus-core-validate` as a repo-native supported-surface evidence command. It runs static contract checks and deterministic runtime smoke, writes JSON evidence under `output/lotus-core-validation/`; CR-1264 wires the PR Merge Gate to check out `lotus-platform` and run the command as a blocking app-validation gate instead of a report-only job. | Let GitHub PR Merge Gate prove the promoted Docker-backed app validation path on Linux after this branch is pushed; keep generated artifacts for diagnosis and avoid treating a stale report-only success as proof. |
...
- CR-1260 focused evidence: ingestion operations helper and event replay app integration tests
  passed with 41 tests; scoped Ruff lint and format checks passed; `repair_ingestion_job_bookkeeping`
  is now `A (3)`, all functions in `ingestion_operations.py` are A-ranked, and the file remains
  `B (12.86)` maintainability pending future file-size modularity work.
- CR-1169 focused evidence: `make quality-integration-lite-collection-gate` collected 126 tests;
...
- CR-1215 migrated instrument enrichment invalid requests away from raw
  `CoreSnapshotBadRequestError` text. Invalid enrichment requests now use
  `QCP_INSTRUMENT_ENRICHMENT_INVALID_REQUEST` with bounded detail and
  `InstrumentReferenceBundle` metadata.
- CR-1216 migrated benchmark market-series invalid requests away from raw `ValueError` text.
  Invalid page-token requests now use `QCP_INTEGRATION_SOURCE_INVALID_REQUEST` with bounded detail
  and `MarketDataWindow` metadata.
- CR-1217 promoted the repeated query-control-plane issue #677 raw-error pattern into
  `make qcp-problem-details-guard`, a deterministic static gate that rejects direct
  FastAPI/Starlette `HTTPException` usage and raw `detail=str(...)` payloads in active
  `query_control_plane_service` routers.
- CR-1242 continued issue #677 by replacing simulation-router exception-message classification
  with typed simulation service errors and type-based QCP problem-details mapping, preserving
  routes, statuses, success DTOs, persistence, Kafka topics, and existing `ValueError`
  compatibility for direct callers.
- CR-1218 fixed issue #594 for direct ingestion publish routers by routing Kafka publish
...
- CR-1243 fixed issue #667 by making the simulation repository staging-only and moving mutation
  unit-of-work ownership, deterministic clock/ID providers, version increments, commit/refresh, and
  missing-delete rollback into `SimulationService`.
