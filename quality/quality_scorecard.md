...
| Dependency usage | Production-source baseline measured after CR-877: `python -m deptry src --extend-exclude "src/services/query_service/build"` reports 928 `DEP003` findings across 485 scanned source files; the report-only CI scope now avoids `.venv` and generated build noise | Make root runtime dependency metadata and first-party package modeling truthful, then ratchet deptry from report-only to an enforceable source gate |
