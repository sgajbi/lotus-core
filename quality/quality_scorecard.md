...
| Typed source-data read records | High-value source-data mappers still accepted broad row objects or ORM tuple shapes, making persistence schema details part of application/source-data contracts. | CR-1257 adds `PortfolioTaxLotReadRecord` and maps `PortfolioTaxLotWindow:v1` repository rows to the typed record before source-data mapping and response assembly. | Keep #664 and #648 open for `PerformanceComponentEconomics:v1` optional cashflow/cost typed records, broader repository output-shape standards, and static/architecture detection of ORM-row leakage. |
