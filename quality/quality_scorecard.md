...
| Query-control-plane error contracts | Representative query-control-plane routers exposed downstream-facing failures as bare `detail` strings, often sourced from raw exception text, so clients lacked stable application error codes, correlation context, and safe metadata. | CR-1191 adds a shared problem-details contract and migrates representative core-snapshot, analytics-input, and simulation failures to top-level problem-details payloads with stable QCP error codes, correlation IDs, bounded details, and safe metadata while preserving HTTP statuses. | Continue migrating remaining control-plane route families and add a deterministic guard once the problem-details migration baseline is broad enough to enforce without noisy exceptions. |
