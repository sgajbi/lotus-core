...
| Async trace context | HTTP request handling produced trace context, but async event paths only propagated `correlation_id`, limiting incident traceability through outbox, Kafka consumers, DLQ, and replay. | CR-1247 adds shared `traceparent` normalization/context, preserves incoming HTTP `traceparent`, extends the governed event envelope with `traceparent`, and propagates it through outbox payloads, Kafka headers, shared consumers, DLQ publication, direct ingestion publish, and DLQ replay with focused unit and integration proof. | Add a future guard/inventory field for direct Kafka topic `traceparent` header support if direct publish coverage expands beyond current focused tests. |
