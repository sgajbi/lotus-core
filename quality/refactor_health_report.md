...
| Typed source-data read records | Improving | CR-1257 adds `PortfolioTaxLotReadRecord` so `PortfolioTaxLotWindow:v1` no longer consumes raw `(PositionLotState, trade_currency)` repository tuples across the application boundary; #664 and #648 remain open for broader source-data and repository output-shape coverage |
