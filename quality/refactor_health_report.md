...
| HTTP observability | Improving | CR-1172 removes raw HTTP request paths and portfolio/security business-key labels from shared Prometheus metrics; CR-1175 routes health-only worker web apps through the standard HTTP bootstrap for `/metrics`, `/health/live`, `/health/ready`, correlation/request/trace headers, route-template HTTP metrics, and request-completion logs; CR-1192 adds centralized metrics access policy enforcement for standard apps and web-backed worker runtime while preserving default private-network scrape compatibility; CR-1226 makes shared readiness dependency checks bounded and failure-isolated with explicit `timeout`, `misconfigured`, and `error` states; CR-1249 adds bounded dependency outcome, latency, and readiness-state metrics for shared database/Kafka readiness checks; CR-1250 adds enforced metric vocabulary, replaces HTTP `path` labels with `endpoint_template`, and requires service-local metric ownership; CR-1251 adds default `BaseConsumer` lifecycle and processing metrics; CR-1252 adds structured operational log taxonomy helpers and a blocking guard for f-string or identifier-bearing operational logger messages in guarded paths |
| Sensitive output redaction | Improving | CR-1173 centralizes structured-log/test-output redaction in `portfolio_common.logging_utils` and fixes #496 locally pending PR/CI/QA; CR-1174 reuses the shared policy for shared Kafka consumer DLQ payloads; CR-1176 routes durable ingestion request payload storage through source-safe redaction and adds canonical fingerprint groundwork |
...
| Post-publish bookkeeping repair | Improving | CR-1228 expands `INGESTION_JOB_BOOKKEEPING_FAILED` with explicit publish/work state, published counts, `retry_safe=false`, recovery action/path, and supportability reason codes, then adds protected evidence-gated `POST /ingestion/jobs/{job_id}/bookkeeping/repair` so operators can repair accepted jobs after publish or persist work completed |
| Ingestion retry recovery responses | Improving | CR-1227 routes ingestion-job retry not-found, unsupported, partial-unsupported, blocked, duplicate, publish-failed, bookkeeping-failed, and audit-write-failed paths through stable recovery details with `outcome`, `remediation`, and `recovery_path`, while preserving side effects and keeping raw downstream exception text out of primary client messages |
...
| Cash-balance account-id provenance | Improving | CR-1200 validates transaction-derived cash-account fallback mappings against active/effective cash-account master data and exposes response-level provenance; CR-1256 classifies unresolved settlement cash-account references at raw transaction persistence as provisional evidence with structured supportability |
| Instrument reference integrity | Improving | CR-1201 defers product cost processing when instrument master data is missing before cost and lot-state writes; CR-1203 and CR-1204 add degraded source-product supportability for transaction-ledger and tax-lot rows with unresolved instruments; CR-1255 makes raw transaction persistence explicitly provisional when instrument master data has not arrived |
...
| Typed source-data read records | Improving | CR-1257 adds `PortfolioTaxLotReadRecord` so `PortfolioTaxLotWindow:v1` no longer consumes raw `(PositionLotState, trade_currency)` repository tuples across the application boundary; CR-1258 adds typed `PerformanceComponentEconomics:v1` transaction, cashflow, and cost read records so optional joined evidence no longer crosses into source-data assembly as ORM relationship shape. #664 is fixed locally pending PR/CI/QA |
| Repository output-shape guard | Improving | CR-1259 adds `make repository-output-shape-guard` and a repository output-shape standard so new public repository methods cannot expose ORM return annotations without a transitional exception, and stale exceptions fail once typed-record conversions land. #648 is fixed locally pending PR/CI/QA |
...
Current continuation: CR-1213 applies the learned QCP pattern to twelve mandate-scoped
client/profile, liquidity, withdrawal, external treasury, and external OMS source routes. Missing
discretionary mandate bindings now use `QCP_INTEGRATION_SOURCE_NOT_FOUND` with source-product,
...
Current continuation: CR-1215 migrates instrument enrichment invalid-request errors away from raw
`CoreSnapshotBadRequestError` text. Invalid enrichment requests now use
`QCP_INSTRUMENT_ENRICHMENT_INVALID_REQUEST` with bounded detail and `InstrumentReferenceBundle`
metadata. HTTP status, route, request/success DTOs, service call, persistence, and source-data
envelope are preserved. Focused proof passed with 3 unit/app/OpenAPI tests.

Current continuation: CR-1216 migrates benchmark market-series invalid-request errors away from raw
`ValueError` text. Invalid page-token requests now use `QCP_INTEGRATION_SOURCE_INVALID_REQUEST`
...
Current continuation: CR-1173 addresses validated GitHub issue #496 by centralizing structured-log
and CI/test-output redaction in `portfolio_common.logging_utils`. Focused
logging/enterprise/output-control tests pass with 20 tests, scoped Ruff lint/format checks pass,
`make security-audit` passes with dependency consistency clean and no known vulnerabilities, and
issue #496 is fixed locally pending PR CI/QA evidence.
...
324. Began validated GitHub issue #653 by moving valuation scheduler dispatch behind a
     `ValuationJobPublisher` port with a Kafka adapter. `ValuationScheduler` no longer imports
...
351. Fixed validated GitHub issue #565 by adding bounded shared health dependency metrics for
     database/Kafka readiness checks: `health_dependency_check_total`,
     `health_dependency_check_duration_seconds`, and `health_readiness_state`. Direct API apps and
     standard health-only worker apps now pass stable service labels into the shared health router,
     cached readiness probes refresh the readiness-state gauge without re-running dependency checks,
     and focused tests prove bounded service/dependency/status/state labels without route paths,
     request identifiers, raw exception text, portfolio IDs, or security IDs. Producer-specific and
     downstream-client readiness dependencies remain a follow-up extension point on the same helper
     because they are not first-class shared health-router dependencies today.
352. Fixed validated GitHub issue #566 by adding a governed metric vocabulary and `make
     metric-vocabulary-guard`. The guard scans production metric definitions, rejects forbidden or
     unknown labels, rejects dynamic label definitions, and requires service-local metrics to be
     registered with an owner. Shared HTTP metrics now use `endpoint_template` instead of `path`,
     aligning the label name with the route-template value already emitted by the standard HTTP
     bootstrap. Focused guard/bootstrap proof passed with 16 tests, and query/ingestion app
     contract metrics proof passed with 60 tests.
353. Fixed validated GitHub issue #567 by moving standard Kafka consumer lifecycle and processing
     telemetry into `BaseConsumer`. All shared consumer subclasses now emit bounded processing
     attempt, success, retryable failure, terminal failure, DLQ success/failure, commit failure,
