...
| `PortfolioManagerBookMembership:v1` | `/integration/portfolio-manager-books/{portfolio_manager_id}/memberships` | PM-book membership resolved from effective accepted portfolio-manager roles, with `advisor_id` retained only for portfolios with no role-assignment history. | Implemented under #513 with explicit authoritative/legacy lineage and approved for the implemented `lotus-gateway` advisor-book facade plus `lotus-manage`. The canonical seed persists the governed `PM_SG_001` assignment and refuses validation until Gateway returns authoritative assignment lineage with current source evidence. Empty or timestamp-free evidence remains fail-closed. Federated discovery and controlled live proof remain closure obligations. It does not claim staff hierarchy, relationship householding, entitlement, advisory authority, or legal ownership. |
