...
| `DpmPortfolioUniverseCandidate:v1` | `/integration/dpm/portfolio-universe/candidates` | Paged source-owned DPM portfolio-universe candidate discovery from effective discretionary mandate bindings, with explicit `EFFECTIVE_DISCRETIONARY_MANDATE_BINDING` selection basis, as-of, booking-center, model-portfolio, active-authority, continuation-token, supportability, snapshot, and lineage evidence. | Implemented locally for RFC37-WTBD-004 source-owner depth. The canonical seed now includes a three-candidate source scenario (`PB_SG_GLOBAL_BAL_001`, `PB_SG_GLOBAL_INC_002`, `PB_SG_GLOBAL_GROWTH_003`) and the live validator requires full continuation-token paging through terminal page exhaustion, rejecting duplicate, empty, or non-terminating continuation pages. It deliberately does not claim relationship-householding, suitability approval, PM ranking, execution readiness, client communication workflow, full analytics support for source-only rows, or external workflow ownership. |
...
does not close broader source-product depth by itself; the source-only candidate seed rows prove
