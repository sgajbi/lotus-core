...
| `ExternalFXForwardCurve:v1` | `/integration/market-data/external-fx-forward-curve` | Fail-closed external treasury FX forward curve posture for currency-overlay workflows. It returns `UNAVAILABLE` with requested currency pairs and tenors echoed for audit, empty curve points, missing external curve data, blocked capabilities, lineage, runtime metadata, and deterministic fingerprints until bank-owned treasury curve ingestion is certified. | Implemented locally for RFC39-WTBD-008 runtime source posture. It is source-owner supportability only and does not claim forward pricing, FX valuation methodology, hedge advice, treasury instructions, counterparty selection, order generation, best execution, venue routing, OMS acknowledgement, fills, settlement, or autonomous treasury action. |
