...
| `DpmPortfolioUniverseCandidate:v1` | `/integration/dpm/portfolio-universe/candidates` | Paged source-owned DPM portfolio-universe candidate discovery from effective discretionary mandate bindings, with explicit `EFFECTIVE_DISCRETIONARY_MANDATE_BINDING` selection basis, as-of, booking-center, model-portfolio, active-authority, continuation-token, supportability, snapshot, lineage, and canonical READY content-identity evidence. | The canonical seed includes a three-candidate source scenario (`PB_SG_GLOBAL_BAL_001`, `PB_SG_GLOBAL_INC_002`, `PB_SG_GLOBAL_GROWTH_003`). The live validator requires a non-placeholder SHA-256 `content_hash`/matching `source_digest` for the complete READY response and full continuation-token paging through terminal exhaustion, rejecting duplicate, empty, or non-terminating pages. `source_batch_fingerprint` remains null without persisted upstream batch evidence. The product does not claim relationship-householding, suitability approval, PM ranking, execution readiness, client communication workflow, full analytics support for source-only rows, or external workflow ownership. |
