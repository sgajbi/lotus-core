...
`ExternalHedgeExecutionReadiness:v1` is active only as a fail-closed unavailable runtime posture:
no external treasury ingestion table is certified and no downstream consumer may claim treasury
policy approval, forward pricing, hedge advice, counterparty selection, best execution, OMS
acknowledgement, fills, settlement, or autonomous treasury action from the current posture. The
other external treasury contracts remain planned domain products only.
