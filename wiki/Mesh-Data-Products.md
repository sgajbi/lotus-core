...
`ExternalCurrencyExposure:v1` and `ExternalHedgeExecutionReadiness:v1` are active only as
fail-closed unavailable runtime postures: no external treasury ingestion table is certified and no
downstream consumer may claim FX attribution, treasury policy approval, forward pricing, hedge
advice, counterparty selection, best execution, OMS acknowledgement, fills, settlement, or
autonomous treasury action from the current posture. The other external treasury contracts remain
planned domain products only.
