...
10. RFC-0083 Slice 6 now defines the priority source-data product catalog, product metadata requirements, consumer map, paging/export disposition, route metadata bindings, DTO-envelope product identity, deterministic reconstruction-scope lineage for holdings, cash, transaction windows, and portfolio snapshots, job-scoped `IngestionEvidenceBundle:v1` on `event_replay_service`, persisted reconciliation finding lifecycle evidence, lifecycle-derived current reconciliation gates that preserve immutable completion summaries, fail-closed `ReconciliationEvidenceBundle:v1` and `DataQualityCoverageReport:v1` gates, a linted source-data product contract guard, and a `lotus-performance` analytics-input consumer conformance guard. Domain-product request scope must describe the primary response grain: `HoldingsAsOf:v1` is a bulk portfolio product, `IngestionEvidenceBundle:v1` is one non-bulk ingestion job, and material route identities such as reconciliation `run_id` and `portfolio_manager_id` must be declared. Response content hashes must never be presented as upstream source-batch identity,
...
13. RFC-0083 Slice 9 now defines source-data product security, tenancy, entitlement, capability, audit, sensitivity, and retention profiles, exposes that posture through guarded `x-lotus-source-data-security` OpenAPI route metadata, prevents operator-only products from drifting outside control-plane/policy route families, constrains access classifications, audit requirements, and sensitivity-driven retention requirements to governed RFC-0082/RFC-0083 lanes, derives default source-data read capability rules from the governed product catalog for both `GET` and query-style `POST` routes, centralizes duplicated query-service/query-control-plane enterprise readiness authorization, policy-header, capability, write-audit, read-audit, read-authorization, and strict capability-rule middleware support in `portfolio_common.enterprise_readiness` while preserving service-local wrappers, and defaults production-like environments to the shared service-local enterprise security profile,
    promoted profiles also persist one typed decision before protected work across ingestion, query,
    query control plane, financial reconciliation, and event replay. Persistence failure stops route
    ...
    store or strict validation, cannot suppress GET/HEAD evidence through
...
11. future market/reference/benchmark/index/risk-free DTO work must use `docs/architecture/RFC-0083-market-reference-data-target-model.md` and `src/libs/portfolio-common/portfolio_common/market_reference_quality.py` before changing observed/source timestamp or freshness/completeness semantics; runtime `data_quality_status`, `reconciliation_status`, `freshness_status`, and publication reasons must derive from the shared fail-closed precedence and must not collapse stale, blocked, or unknown evidence into structural `PARTIAL`,
...
195. `transactions.raw.received` / `persistence_group_transactions` and
     ...
     monotonic transaction timestamp/sequence across profile phases and supported reused-stack
     runs. Start after the fixture portfolio's latest persisted transaction; a burst capacity phase
     must append to steady-state history rather than reset its order and fabricate retroactive
     rebuild work. Keep observation windows separate from enforced drain SLOs, count only
     authoritatively accepted replay commands, and retain final domain counts when a drain times out.
...
222. Broad DB-direct pytest cleanup is authorized only for a `PreparedTestRuntime` generated by the
     current process with a generated Compose project, a dynamically reserved PostgreSQL host port,
     and an actual SQLAlchemy engine matching the immutable, credential-free user, host, port, and
     database target captured by the preparation factory. Mutable values remain exported for
     Compose/subprocess compatibility, but post-preparation project, PostgreSQL component, or
     ...
     each destructive connection, after metadata discovery, and again immediately before every
     destructive SQL statement. Replay-only recovery requires the
...
227. Shared outbox dispatch preserves ordering per `(topic, partition_key)` through a durable
     ...
     the governed safety margin; start that lease after stream-head selection so query latency
     cannot consume the publication margin, and reject unsafe overrides at startup. A flush
     exception or nonzero flush result must purge queued and in-flight messages and replace the
     underlying producer before releasing any affected claim; if purge confirmation fails, abort
     result persistence so delivery uncertainty cannot advance the stream. Every production
     dispatcher must own a fresh, non-cached producer;
     ...
     budget by the governed termination margin; validate the final combined consumer/dispatcher
     budget before starting worker tasks and reject unsafe claim lease, supervision, or termination
     combinations at startup. The governed dispatcher
...
246. Latest position, position-history, and valuation-snapshot reads on PostgreSQL must use
     ...
     snapshot-only key into a history-driven replay backlog that can never advance. Register it at
     its latest actual snapshot with explicit `SNAPSHOT_ONLY` posture, exclude that posture from
     scheduler selection and defensive planner normalization, and expose it truthfully on support
...
248. App-local Kafka startup keeps the real `kafka-topics --bootstrap-server kafka:9093 --list`
...
249. Canonical `PB_SG_GLOBAL_BAL_001` valuation is scoped to tenant `LOTUS_PB_SG` and legal book
     ...
     and its explicit inputs into the source content hash. All canonical
...
254. Required merge checks are governed by
     ...
     comparison, require blocking jobs to be unconditional, scan every workflow for static or
     dynamic required-context collisions, reject non-bare workflow name expressions, bind manifest
     authority only to GitHub Actions app `15368`, and fail on missing, stale, duplicate, ambiguous,
     redirected, or undeclared contexts. `make lint` must retain both the
