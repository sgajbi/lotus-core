...
17. API surface truth now has a generated implementation-backed route catalog at
    ...
    drift,
18. the repository already enforces a broad banking-grade CI contract including architecture, OpenAPI, warning, coverage, latency, Docker, and operational gates,
19. canonical shared infrastructure ownership now lives in `lotus-platform`, while `lotus-core` still supports app-local stacks for isolated development,
20. app-local compose keeps the ingestion service write payload cap at 16 MiB through
...
21. RFC-0086 repo-native domain-product declarations now live under
...
22. RFC-0087 trust telemetry proof currently covers exactly `PortfolioStateSnapshot:v1` and
...
23. RFC-0087 now promotes the first DPM source-data products for `lotus-manage` stateful
    ...
    OMS acknowledgement.
24. RFC-0108 portfolio readiness supportability now publishes the bounded
...
25. App-level supported-surface validation now has a single repo-native evidence command:
...
26. Boundary mapping conformance now has a repo-native command,
    ...
    list ordering.
27. Reference-data ingestion source-observation lineage now has a shared DTO contract for
...
28. Consumer DLQ and replay-audit evidence now follows a correlation-or-reason contract:
    ...
    replay responses.
29. Replay audit recording is mandatory for ingestion-job retry and consumer-DLQ replay workflows.
    ...
    governed endpoint.
30. Direct ingestion-router Kafka publish dependency failures use the shared
...
31. Direct ingestion post-publish or post-persist bookkeeping failures must preserve the
...
32. Ingestion job retry recovery failures must use stable recovery details with `code`, `message`,
    ...
    status.
33. `/metrics` is an operational scrape endpoint governed by the shared metrics access policy.
...
34. Shared `/health/ready` endpoints must keep dependency checks bounded and failure-isolated
...
35. Prometheus metric vocabulary is governed by `portfolio_common.observability_contracts` and
...
36. Standard FastAPI service apps and health-only worker web apps are covered by the shared HTTP
...
37. Kafka consumers inheriting `portfolio_common.kafka_consumer.BaseConsumer` emit the standard
...
38. Structured operational logging is governed by
...
39. Query-control-plane routes (QCP routes under `query_control_plane_service`) migrated to the
...
40. Runtime configuration is becoming strict outside local/development/test profiles. Invalid
...
41. Service runtime import correctness is now part of the architecture guard. Service code under
...
42. Canonical transaction-type classification now starts in
