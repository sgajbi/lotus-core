...
220. Canonical front-office seed success requires stable terminal derived state. Pending,
 ...
 222. Broad DB-direct pytest cleanup is authorized only for a `PreparedTestRuntime` generated by the
     ...
     and an actual SQLAlchemy engine matching the exact prepared user, host, port, and database.
     ...
     capability explicitly. Inherited app-local/shared projects, fixed ports, test-shaped project
     names, mixed provenance, and post-preparation engine drift fail closed; prefixes and two values
     derived from the same inherited environment are not ownership proof.
