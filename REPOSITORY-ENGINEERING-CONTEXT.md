...
9. RFC-0083 Slice 6 now defines the priority source-data product catalog, product metadata requirements, consumer map, paging/export disposition, route metadata bindings, DTO-envelope product identity, HoldingsAsOf runtime data-quality metadata, TransactionLedgerWindow runtime evidence timestamp metadata, PortfolioStateSnapshot runtime metadata and freshness epoch handling, analytics-input, market/reference, coverage data-quality classification, and ingestion/reconciliation evidence runtime supportability metadata with reconciliation evidence status derivation, and a linted source-data product contract guard,
