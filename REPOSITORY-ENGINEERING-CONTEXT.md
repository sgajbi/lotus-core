...
12. RFC-0083 Slice 9 now defines source-data product security, tenancy, entitlement, audit, sensitivity, and retention profiles, exposes that posture through guarded `x-lotus-source-data-security` OpenAPI route metadata, prevents operator-only products from drifting outside control-plane/policy route families, constrains access classifications and audit requirements to governed RFC-0082 route-family lanes, and centralizes duplicated query-service/query-control-plane enterprise readiness authorization, policy-header, capability, write-audit, and opt-in read-audit middleware support in `portfolio_common.enterprise_readiness` while preserving service-local wrappers,
