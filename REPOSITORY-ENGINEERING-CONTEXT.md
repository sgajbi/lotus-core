...
- Tenant S1 and the transaction event/idempotency fence are implemented, but #798 S2-S6 remain
  open. Do not claim estate-wide tenant isolation until outbox ownership, remaining stateful
  fences, ledger and derived records, query paths, replay, and operations are tenant-bound and
  exact-main proven. Global market-data fences remain explicitly global.
