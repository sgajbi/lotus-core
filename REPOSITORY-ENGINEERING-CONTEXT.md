...
13. RFC-0083 Slice 10 now defines event family governance, schema governance requirements, operator supportability surface posture, and a guarded runtime outbox event/type topic alignment check,
...
13. future event, outbox, replay, DLQ, and operator diagnostic changes must use `docs/architecture/RFC-0083-eventing-supportability-target-model.md` and `src/libs/portfolio-common/portfolio_common/event_supportability.py`, and must pass `make event-runtime-contract-guard` when outbox emissions or topics are touched,
