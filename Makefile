...
main-gate-coverage-audit:
	$(REPOSITORY_PYTHON) scripts/release/audit_main_gate_coverage.py --baseline-ref refs/tags/main-gate-coverage-enforcement-v1 --limit 400 --output output/release-evidence/main-gate-coverage.json --fail-on-gap
...
refresh-technology-governance-receipts:
	$(REPOSITORY_PYTHON) scripts/development/renew_technology_governance_receipts.py --run-id $(RUN_ID)
...
reprocessing-transition-boundary-guard:
	$(REPOSITORY_PYTHON) scripts/quality/reprocessing_transition_boundary_guard.py
...
test-query-authority-db-contract:
	$(REPOSITORY_PYTHON) scripts/quality/test_manifest.py --suite query-authority-db-contract --quiet
