...
def run_recovery_gate(args: argparse.Namespace) -> KafkaRestartRecoveryEvidence:
    ...
    with managed:
        ...
        _run(
            dependency_gated_recovery_command(managed.compose_command),
            environment=environment,
        )
...
def dependency_gated_recovery_command(
    command_builder: Callable[..., list[str]],
) -> list[str]:
    """Start recovery through topic creator's real service-healthy dependency."""

    return command_builder("up", "-d", "--force-recreate", TOPIC_CREATOR_SERVICE)
