...
def test_dependency_health_cache_is_reused_only_before_merge() -> None:
    for workflow_path in (
        ...
        assert key_step["run"] == "make dependency-health-cache-key"
...
def test_lint_scans_the_complete_repository_python_scope() -> None:
    ...
    assert target_dependencies["lint"] == [
        "quality-ruff-gate",
...
def test_runtime_latency_gates_use_bounded_demo_seed_history() -> None:
    for workflow_path in GOVERNED_RUNTIME_WORKFLOWS:
        workflow_text = workflow_path.read_text(encoding="utf-8")
        assert 'DEMO_DATA_PACK_HISTORY_DAYS: "240"' in workflow_text
        assert 'DEMO_DATA_PACK_PORTFOLIO_IDS: "DEMO_DPM_EUR_001"' in workflow_text
        assert 'DEMO_DATA_PACK_INGEST_ONLY: "true"' in workflow_text
