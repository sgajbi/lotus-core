...
def test_metadata_detects_ignored_symlink_lexically_within_copied_source(
    ...
    tmp_path.joinpath(".git", "info", "exclude").write_text(
        "src/services/query_service/app/local-link\n", encoding="utf-8"
    )
    app.joinpath("local-link").symlink_to(external)
...
def test_metadata_detects_untracked_empty_copied_source_root(tmp_path: Path) -> None:
    _write_project(tmp_path)
    service = tmp_path / "src" / "services" / "query_service"
    service.mkdir(parents=True)
    dockerfile = service / "Dockerfile"
    dockerfile.write_text(
        "FROM scratch\nCOPY src/services/query_service/extensions /extensions\n",
        encoding="utf-8",
    )
    _run_git(tmp_path, "init", "--initial-branch", "main")
    _run_git(tmp_path, "config", "user.name", "Local Build Test")
    ...
    _run_git(tmp_path, "commit", "-m", "test fixture")
    expected_head = _run_git(tmp_path, "rev-parse", "HEAD")
    service.joinpath("extensions").mkdir()
...
def test_metadata_detects_ignored_file_in_json_form_copy(tmp_path: Path) -> None:
    _write_project(tmp_path)
    service = tmp_path / "src" / "services" / "query_service"
    app = service / "app"
    app.mkdir(parents=True)
    ...
    expected_head = _run_git(tmp_path, "rev-parse", "HEAD")
    tmp_path.joinpath(".git", "info", "exclude").write_text(
        "src/services/query_service/app/local-extension.py\n", encoding="utf-8"
    )
    app.joinpath("local-extension.py").write_text("local = 1\n", encoding="utf-8")
...
def test_metadata_detects_ignored_directly_copied_file(tmp_path: Path) -> None:
    _write_project(tmp_path)
    service = tmp_path / "src" / "services" / "query_service"
    service.mkdir(parents=True)
    service.joinpath("Dockerfile").write_text(
        'FROM scratch\nCOPY ["src/local.py", "/app/local.py"]\n',
        encoding="utf-8",
    )
    _run_git(tmp_path, "init", "--initial-branch", "main")
    _run_git(tmp_path, "config", "user.name", "Local Build Test")
    ...
    _run_git(tmp_path, "commit", "-m", "test fixture")
    expected_head = _run_git(tmp_path, "rev-parse", "HEAD")
    tmp_path.joinpath(".git", "info", "exclude").write_text("src/local.py\n", encoding="utf-8")
    tmp_path.joinpath("src", "local.py").write_text("local = 1\n", encoding="utf-8")
...
def test_metadata_accepts_local_stage_copy_source(tmp_path: Path, stage_reference: str) -> None:
    _write_project(tmp_path)
    service = tmp_path / "src" / "services" / "query_service"
    service.mkdir(parents=True)
    service.joinpath("Dockerfile").write_text(
        ...
        "RUN touch /wheels\n"
        "FROM scratch\n"
        f"COPY --from={stage_reference} /wheels /wheels\n",
        encoding="utf-8",
    )
    _run_git(tmp_path, "init", "--initial-branch", "main")
    _run_git(tmp_path, "config", "user.name", "Local Build Test")
...
@pytest.mark.parametrize("stage_reference", ["external", "2"])
def test_metadata_rejects_external_stage_copy_source(tmp_path: Path, stage_reference: str) -> None:
    _write_project(tmp_path)
    service = tmp_path / "src" / "services" / "query_service"
    service.mkdir(parents=True)
    service.joinpath("Dockerfile").write_text(
        f"FROM scratch\nCOPY --from={stage_reference} /artifact /app/artifact\n",
        encoding="utf-8",
    )
    _run_git(tmp_path, "init", "--initial-branch", "main")
    _run_git(tmp_path, "config", "user.name", "Local Build Test")
...
def test_metadata_rejects_context_bind_mount(tmp_path: Path) -> None:
    _write_project(tmp_path)
    service = tmp_path / "src" / "services" / "query_service"
    service.mkdir(parents=True)
    service.joinpath("Dockerfile").write_text(
    ...
    with pytest.raises(ValueError, match="context bind mounts are not supported"):
        discover_local_build_metadata(tmp_path)
