...
def test_image_provenance_guard_rejects_add_instruction(tmp_path: Path) -> None:
    _write_required_sources(tmp_path)
    _write_dockerfile(
        tmp_path,
...
def test_image_provenance_guard_accepts_harmless_double_angle_text(tmp_path: Path) -> None:
    ...
    assert not any(
        "Dockerfile heredocs are not permitted" in finding.detail for finding in findings
    )


def test_image_provenance_guard_rejects_nondefault_escape_directive(tmp_path: Path) -> None:
    _write_required_sources(tmp_path)
    _write_dockerfile(tmp_path, "# escape=`\n" + _complete_dockerfile())

    findings = find_image_provenance_findings(tmp_path)

    assert any("non-default Dockerfile escape directives" in finding.detail for finding in findings)
...
def test_image_provenance_guard_rejects_compose_provenance_label_override(
    tmp_path: Path,
) -> None:
    _write_required_sources(tmp_path)
    _write_dockerfile(tmp_path, _complete_dockerfile())
    ...
    compose.write_text(
        compose.read_text(encoding="utf-8").replace(
            ...
            "      context: .\n"
            "      labels:\n"
            "        org.opencontainers.image.revision: fabricated\n",
        ),
        encoding="utf-8",
    )
    ...
    assert any(
        "overrides provenance label org.opencontainers.image.revision" in finding.detail
        for finding in findings
    )
...
def test_image_provenance_guard_rejects_inherited_compose_build(tmp_path: Path) -> None:
    _write_required_sources(tmp_path)
    _write_dockerfile(tmp_path, _complete_dockerfile())
    compose = tmp_path / "docker-compose.yml"
    compose.write_text(
        compose.read_text(encoding="utf-8").replace(
            ...
            "      service: ungoverned-query-service\n    build:\n",
        ),
        encoding="utf-8",
    )
