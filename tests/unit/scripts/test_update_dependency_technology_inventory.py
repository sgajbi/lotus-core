...
def test_license_classification_rejects_declared_expression_with_ambiguous_classifier() -> None:
    result = inventory._license_classification(
        {
    ...
    assert result["classification"] == "review_required"
    assert result["classification_reason"] == "ambiguous_classifier_with_declared_expression"


def test_license_classification_rejects_declared_expression_classifier_conflict() -> None:
    policy = _policy()
    policy["approved_single_spdx_expressions"].append("BSD-3-Clause")  # type: ignore[union-attr]
    policy["classifier_mappings"]["License :: OSI Approved :: BSD License"] = "BSD-3-Clause"  # type: ignore[index]
    result = inventory._license_classification(
    ...
    assert result["classification"] == "review_required"
    assert result["classification_reason"] == "conflicting_declared_and_classifier_metadata"
