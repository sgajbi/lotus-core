...
def test_dpm_planned_source_data_security_profiles_are_system_scoped() -> None:
    ...
    hedge_policy_profile = get_source_data_security_profile("ExternalHedgePolicy")
    assert hedge_policy_profile.sensitivity_classification == CLIENT_SENSITIVE
    assert hedge_policy_profile.audit_requirement == AUDIT_SYSTEM_ACCESS
...
def test_client_source_products_classify_sensitive_identifiers() -> None:
    ...
    cash_movement_profile = get_source_data_security_profile("PortfolioCashMovementSummary")
    assert cash_movement_profile.sensitivity_classification == CLIENT_SENSITIVE
    assert {"portfolio_id", "client_id"} <= set(cash_movement_profile.pii_fields)
