...
from src.services.portfolio_aggregation_service.app.application import (
    stage_portfolio_aggregation_completion,
)
...
async def test_use_case_delegates_domain_completion_to_event_port() -> None:
    ...
    use_case = stage_portfolio_aggregation_completion.StagePortfolioAggregationCompletion(
        event_stager=stager
    )
    completion = PortfolioAggregationCompletion(
        portfolio_id="PORT-1",
