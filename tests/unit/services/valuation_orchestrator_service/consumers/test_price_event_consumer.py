...
async def test_current_price_without_ready_open_positions_relies_on_position_readiness(
    consumer: PriceEventConsumer,
    mock_kafka_message: MagicMock,
    mock_event: MarketPricePersistedEvent,
    mock_dependencies: dict,
...
async def test_price_event_uses_header_correlation_for_direct_processing(
    ...
    queued_job = mock_job_repo.upsert_jobs.await_args.args[0][0]
    assert queued_job.correlation_id == "test-corr-id"
    assert queued_job.source_correction_id == _price_source_correction_id(mock_event)
