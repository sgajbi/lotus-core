...
async def test_get_session_returns_immutable_record(db: AsyncMock):
    ...
    session = await SqlAlchemySimulationStore(db).get_session(tenant_id="tenant-a", session_id="S1")
...
async def test_stage_session_close_updates_status_and_version(db: AsyncMock):
    await SqlAlchemySimulationStore(db).stage_session_close(
        tenant_id="tenant-a", session_id="S1", version=3
    )
...
async def test_stage_changes_rejects_session_from_another_tenant(db: AsyncMock):
    with pytest.raises(ValueError, match="session tenant does not match"):
        await SqlAlchemySimulationStore(db).stage_changes(
            _session(),
            tenant_id="tenant-b",
            version=3,
            changes=[],
...
async def test_stage_change_delete_returns_false_without_version_update(db: AsyncMock):
    ...
    deleted = await SqlAlchemySimulationStore(db).stage_change_delete(
        tenant_id="tenant-a", session_id="S1", change_id="C404", version=3
    )
...
async def test_get_changes_returns_ordered_immutable_records(db: AsyncMock):
    ...
    changes = await SqlAlchemySimulationStore(db).get_changes(tenant_id="tenant-a", session_id="S1")
...
async def test_portfolio_exists_uses_bounded_identity_query(db: AsyncMock):
    ...
    exists = await SqlAlchemySimulationBaselineReader(db).portfolio_exists(
...
async def test_current_positions_map_reconciled_snapshot_rows(db: AsyncMock):
    ...
    positions = await SqlAlchemySimulationBaselineReader(db).get_current_positions(
        tenant_id="tenant-a", portfolio_id="P1"
    )
...
async def test_current_positions_fall_back_to_current_epoch_history(db: AsyncMock):
    ...
    assert "portfolios.tenant_id = 'tenant-a'" in history_sql
