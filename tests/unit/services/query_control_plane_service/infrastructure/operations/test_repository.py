...
from src.services.query_control_plane_service.app.domain.operations import (
...
def _reconciliation_run_row() -> SimpleNamespace:
    return SimpleNamespace(
        run_id="recon-001",
...
def _lineage_key_row(security_id: str = "S1") -> dict[str, object]:
    return {
        "security_id": security_id,
        "epoch": 2,
        "watermark_date": date(2025, 8, 29),
        "reprocessing_status": "CURRENT",
        "latest_position_history_date": date(2025, 8, 30),
        "latest_daily_snapshot_date": date(2025, 8, 30),
        "latest_valuation_job_date": date(2025, 8, 30),
        "latest_valuation_job_id": 81,
        "latest_valuation_job_status": "COMPLETED",
        "latest_valuation_job_correlation_id": "corr-081",
    }
...
async def test_get_reprocessing_health_summary(
    ...
    result = MagicMock()
    result.one.return_value = mock_row
    ...
    assert value.oldest_reprocessing_updated_at == datetime(2025, 8, 20, 9, 0, tzinfo=timezone.utc)
    stmt = mock_db_session.execute.call_args[0][0]
    compiled = str(stmt.compile(compile_kwargs={"literal_binds": True}))
...
async def test_get_analytics_export_job_health_summary(
    ...
    assert "status = 'accepted'" in compiled
...
async def test_get_snapshot_valuation_coverage_summary_honors_snapshot_as_of(
    ...
    assert "daily_position_snapshots.valuation_status != 'UNVALUED'" in compiled
    assert "upper(trim(daily_position_snapshots.valuation_status))" not in compiled
...
async def test_get_latest_transaction_date(
    ...
    mock_execute_scalar_one_or_none(
        mock_db_session,
        datetime(2025, 8, 31, 15, 45, tzinfo=timezone.utc),
    )
    ...
    stmt = mock_db_session.execute.call_args[0][0]
    compiled = str(stmt.compile(compile_kwargs={"literal_binds": True}))
    assert "max(transactions.transaction_date)" in compiled.lower()
    assert "transactions.portfolio_id = 'P1'" in compiled


async def test_get_latest_transaction_date_honors_as_of(
    repository: OperationsRepository, mock_db_session: AsyncMock
):
    mock_execute_scalar_one_or_none(
        mock_db_session,
    ...
    value = await repository.get_latest_transaction_date("P1", as_of=as_of)

    assert value == date(2025, 8, 31)
    stmt = mock_db_session.execute.call_args[0][0]
    compiled = str(stmt.compile(compile_kwargs={"literal_binds": True}))
    assert "transactions.created_at <= '2025-08-30 11:00:00+00:00'" in compiled
...
async def test_get_latest_transaction_date_as_of(
    ...
    mock_execute_scalar_one_or_none(
        mock_db_session,
        datetime(2025, 8, 15, 15, 45, tzinfo=timezone.utc),
    )
...
async def test_get_latest_snapshot_date_for_current_epoch_honors_as_of(
    repository: OperationsRepository, mock_db_session: AsyncMock
):
    mock_execute_scalar_one_or_none(mock_db_session, date(2025, 8, 30))
    as_of = datetime(2025, 8, 30, 11, 0, tzinfo=timezone.utc)

    value = await repository.get_latest_snapshot_date_for_current_epoch("P1", as_of=as_of)
...
async def test_get_position_snapshot_history_mismatch_count_honors_as_of(
    ...
    value = await repository.get_position_snapshot_history_mismatch_count("P1", as_of=as_of)
...
async def test_get_lineage_keys_query(repository: OperationsRepository, mock_db_session: AsyncMock):
    mock_result = MagicMock()
    expected = [_lineage_key_row("S1"), _lineage_key_row("S2")]
    mock_result.mappings.return_value.all.return_value = expected
    mock_db_session.execute = AsyncMock(return_value=mock_result)
...
async def test_get_valuation_jobs_query_by_job_id_does_not_hide_superseded_rows(
    ...
    value = await repository.get_valuation_jobs(portfolio_id="P1", skip=0, limit=20, job_id=8801)
...
async def test_get_latest_financial_reconciliation_control_stage(
    ...
    assert value == PortfolioControlStageEvidence(
        stage_id=701,
        business_date=date(2025, 8, 30),
        epoch=2,
        status="COMPLETED",
...
async def test_get_latest_reconciliation_run_for_portfolio_day(
    ...
    mock_run = _reconciliation_run_row()
...
async def test_get_reconciliation_runs_count_with_filters(
    ...
    assert (
        "financial_reconciliation_runs.requested_by = 'pipeline_orchestrator_service'" in compiled
    )
    assert (
