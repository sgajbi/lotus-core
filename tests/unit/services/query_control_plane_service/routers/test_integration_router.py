...
async def test_resolve_portfolio_benchmark_assignment_success_path() -> None:
    ...
    assert mock_service.resolve.await_args.kwargs["tenant_id"] == "tenant-sg"


@pytest.mark.asyncio
async def test_resolve_portfolio_benchmark_assignment_rejects_tenant_mismatch_before_io() -> None:
    mock_service = MagicMock(spec=BenchmarkAssignmentService)
    mock_service.resolve = AsyncMock()
...
@pytest.mark.asyncio
async def test_pm_book_refuses_mismatched_tenant_before_read() -> None:
    mock_service = MagicMock(spec=PortfolioManagerBookService)
    mock_service.resolve_membership = AsyncMock()
...
@pytest.mark.asyncio
async def test_resolve_cio_model_change_affected_cohort_refuses_mismatched_tenant_before_read() -> (
    None
):
    mock_service = MagicMock(spec=DpmPortfolioPopulationService)
    mock_service.resolve_cio_model_change_cohort = AsyncMock()
...
@pytest.mark.asyncio
async def test_resolve_dpm_portfolio_universe_refuses_mismatched_tenant_before_read() -> None:
    mock_service = MagicMock(spec=DpmPortfolioPopulationService)
    mock_service.resolve_universe_candidates = AsyncMock()
...
async def test_get_client_tax_profile_router_function() -> None:
    mock_service = MagicMock(spec=ClientTaxProfileService)
...
@pytest.mark.asyncio
async def test_client_tax_profile_maps_missing_binding_to_problem_details() -> None:
    mock_service = MagicMock(spec=ClientTaxProfileService)
    mock_service.get_client_tax_profile = AsyncMock(return_value=None)
...
async def test_get_external_eligible_hedge_instruments_router_function() -> None:
    ...
    mock_service.get_external_eligible_hedge_instruments = AsyncMock(
        return_value={
            ...
            "lineage": {"contract_version": "rfc_039_external_eligible_hedge_instrument_v1"},
        }
    )
    ...
    mock_service.get_external_eligible_hedge_instruments.assert_awaited_once_with(
        portfolio_id="PB_SG_GLOBAL_BAL_001",
        request=request,
    )
...
async def test_fetch_benchmark_composition_window_router_functions() -> None:
    mock_service = MagicMock(spec=BenchmarkCompositionService)
    mock_service.resolve = AsyncMock(
        return_value={
            "benchmark_id": "BMK_GLOBAL_BALANCED_60_40",
            "benchmark_currency": "USD",
    ...
    response = await fetch_benchmark_composition_window(
        ...
        benchmark_composition_service=mock_service,
    )
...
async def test_reference_router_success_paths_cover_all_endpoints() -> None:
    ...
    benchmark_composition_response = await fetch_benchmark_composition_window(
        ...
        benchmark_composition_service=composition_service,
    )
    assert benchmark_composition_response["benchmark_currency"] == "USD"
    composition_service.resolve.assert_awaited_once()
