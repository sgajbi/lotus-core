...
from src.services.query_control_plane_service.app.application.core_snapshot.calculations import (
...
async def test_core_snapshot_service_accepts_explicit_dependencies_without_session() -> None:
    ...
    simulation_store = AsyncMock()
    clock = _FixedClock(datetime(2026, 2, 27, tzinfo=UTC))
    ...
    assert service._source_reader is source_reader
    assert service._simulation_store is simulation_store
...
def _position_source(
    ...
    return CoreSnapshotPositionSource(
        ...
        state_status=str(getattr(state, "status", "CURRENT")),
...
def _service(mock_dependencies, *, clock=None) -> CoreSnapshotService:
    position_repo, portfolio_repo, simulation_store, price_repo, fx_repo, instrument_repo = (
        mock_dependencies
    )
    ...
    return CoreSnapshotService(
        CoreSnapshotDependencies(
            source_reader=source_reader,
            simulation_store=simulation_store,
            clock=clock or _FixedClock(datetime(2026, 2, 27, 12, tzinfo=UTC)),
        )
    )
...
async def test_core_snapshot_lifecycle_fences_reprocessing_and_restores_replacement_valuation(
    mock_dependencies,
) -> None:
    position_repo, _, _, _, _, _ = mock_dependencies
    source = position_repo.get_latest_positions_by_portfolio_as_of_date.return_value[0]
    position_repo.get_latest_positions_by_portfolio_as_of_date.return_value = [
        replace(source, state_status="REPROCESSING")
    ]
    ...
    assert response.valuation_context.reason_code == "POSITION_STATE_NOT_CURRENT"

    replacement_timestamp = datetime(2026, 2, 27, 11, tzinfo=UTC)
    position_repo.get_latest_positions_by_portfolio_as_of_date.return_value = [
        replace(
            source,
            state_status="CURRENT",
            source_created_at=replacement_timestamp,
            source_updated_at=replacement_timestamp,
            state_updated_at=replacement_timestamp,
        )
    ]

    replacement = await _service(mock_dependencies).get_core_snapshot(
        "PORT_001",
        CoreSnapshotRequest(
            as_of_date="2026-02-27",
            sections=[CoreSnapshotSection.POSITIONS_BASELINE],
    ...
    assert replacement.reconciliation_status == COMPLETE
    assert replacement.data_quality_status == COMPLETE
    assert replacement.source_evidence_current is True
    assert replacement.freshness_status == "CURRENT"
    assert replacement.valuation_context.supportability == "READY"
    assert replacement.valuation_context.reason_code == "SOURCE_EVIDENCE_READY"
...
async def test_core_snapshot_stale_baseline_valuation_invalidates_readiness(
    mock_dependencies,
):
    (position_repo, _, _, _, _, _) = mock_dependencies
    current = position_repo.get_latest_positions_by_portfolio_as_of_date.return_value[0]
    position_repo.get_latest_positions_by_portfolio_as_of_date.return_value = [
        replace(current, valuation_status="VALUED_STALE")
    ]
    service = _service(mock_dependencies)
