...
from src.services.query_control_plane_service.app.application.analytics.analytics_quality import (
...
from src.services.query_control_plane_service.app.infrastructure.analytics_unit_of_work import (
...
@pytest.mark.asyncio
async def test_get_portfolio_timeseries_position_path_defaults_snapshot_epoch_and_pages() -> None:
    service = make_service()
    service.repo = SimpleNamespace(
        get_portfolio=AsyncMock(
            return_value=SimpleNamespace(
...
async def test_create_export_job_completed() -> None:
    ...
    service.export_repo = SimpleNamespace(
        ...
        mark_running=AsyncMock(
            side_effect=lambda *_args, **_kwargs: (setattr(row, "status", "running"), row)[1]
        ),
        mark_completed=AsyncMock(
            side_effect=lambda *_args, **_kwargs: (setattr(row, "status", "completed"), row)[1]
        ),
...
async def test_create_export_job_marks_failed_on_input_error() -> None:
    ...
    service.export_repo = SimpleNamespace(
        ...
        create_job=AsyncMock(return_value=row),
        get_job=AsyncMock(return_value=row),
        mark_running=AsyncMock(return_value=row),
        mark_completed=AsyncMock(),
        mark_failed=AsyncMock(
            side_effect=lambda *_a, **_k: (setattr(row, "status", "failed"), row)[1]
        ),
    )
...
async def test_get_export_result_json_success_and_export_job_state_helpers() -> None:
    ...
    service.export_repo = SimpleNamespace(
        ...
        mark_running=AsyncMock(return_value=running_row),
        mark_completed=AsyncMock(return_value=running_row),
        mark_failed=AsyncMock(return_value=running_row),
    )
