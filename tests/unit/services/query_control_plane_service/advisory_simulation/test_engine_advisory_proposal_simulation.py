...
from src.services.query_control_plane_service.app.application.advisory_simulation.advisory_engine import (  # noqa: E501
...
def test_proposal_simulation_run_id_uses_explicit_unscoped_identity_without_hash():
    portfolio = portfolio_snapshot(
    ...
    assert result.proposal_run_id == "pr_unscoped"
...
def test_proposal_simulation_reconciliation_mismatch_blocks_run():
    ...
    with patch(
        "src.services.query_control_plane_service.app.application.advisory_simulation.advisory_engine.build_reconciliation",
        return_value=(
            Reconciliation(
...
def test_proposal_simulation_forces_pending_status_when_rule_derivation_returns_ready():
    ...
    with patch(
        "src.services.query_control_plane_service.app.application.advisory_simulation.advisory_engine.derive_status_from_rules",
        return_value="READY",
    ):
        result = run_proposal_simulation(
            portfolio=portfolio,
...
def test_proposal_simulation_skips_buy_intent_without_notional():
    ...
    with (
        patch(
            "src.services.query_control_plane_service.app.application.advisory_simulation.advisory_engine.build_proposal_security_trade_intent",
            return_value=(
                SecurityTradeIntent(
                    intent_id="oi_1",
                    instrument_id="EQ_1",
        ...
        patch(
            "src.services.query_control_plane_service.app.application.advisory_simulation.advisory_engine.build_auto_funding_plan",
