...
def _fx_forward_transaction() -> BookedTransaction:
...
def test_enrich_fx_metadata_populates_defaults_for_forward() -> None:
    transaction = replace(
        _fx_forward_transaction(),
        transaction_type=" fx_forward ",
        component_type=" fx_contract_open ",
        spot_exposure_model=" none ",
        fx_realized_pnl_mode=" none ",
    )
    enriched = enrich_fx_transaction_metadata(transaction)
...
def test_enrich_fx_metadata_populates_swap_defaults() -> None:
    transaction = replace(_fx_forward_transaction(), transaction_type=" fx_swap ")
    enriched = enrich_fx_transaction_metadata(transaction)
