...
from services.calculators.position_valuation_calculator.app.core.valuation_scheduler import (
    ValuationScheduler,
)
...
def mock_dependencies():
    ...
    with (
        patch(
            "services.calculators.position_valuation_calculator.app.core.valuation_scheduler.get_async_db_session",
            new=get_session_gen,
        ),
        patch(
            "services.calculators.position_valuation_calculator.app.core.valuation_scheduler.ValuationRepository",
            return_value=mock_repo,
        ),
        patch(
            "services.calculators.position_valuation_calculator.app.core.valuation_scheduler.ValuationJobRepository",
            return_value=mock_job_repo,
        ),
        patch(
            "services.calculators.position_valuation_calculator.app.core.valuation_scheduler.PositionStateRepository",
            return_value=mock_state_repo,
        ),
        patch(
            "services.calculators.position_valuation_calculator.app.core.valuation_scheduler.ReprocessingJobRepository",
...
async def test_scheduler_creates_position_aware_backfill_jobs(
    scheduler: ValuationScheduler, mock_dependencies: dict
):
    """
...
async def test_scheduler_skips_jobs_for_keys_with_no_position_history(
    ...
    states_to_backfill = [
        PositionState(
            portfolio_id="P1", security_id="S1", watermark_date=date(2025, 8, 10), epoch=1
        )
    ]
...
async def test_scheduler_dispatches_claimed_jobs(
    scheduler: ValuationScheduler, mock_kafka_producer: MagicMock
):
    """
...
async def test_scheduler_updates_pending_triggers_metric(
    mock_gauge_set, scheduler: ValuationScheduler, mock_dependencies: dict
):
    """
