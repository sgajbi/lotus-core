...
from src.services.query_service.app.services.external_eligible_hedge_instrument import (
...
def _request() -> ExternalEligibleHedgeInstrumentRequest:
    return ExternalEligibleHedgeInstrumentRequest(
        as_of_date=date(2026, 5, 3),
        tenant_id="default",
        mandate_id="MANDATE_PB_SG_GLOBAL_BAL_001",
        reporting_currency="USD",
        exposure_currencies=["JPY", "EUR"],
        instrument_types=["FX_SWAP", "FX_FORWARD"],
    )


def test_build_external_eligible_hedge_instrument_response_fails_closed() -> None:
    response = build_external_eligible_hedge_instrument_response(
        portfolio_id="PB_SG_GLOBAL_BAL_001",
        binding=_binding(),
        request=_request(),
    )
    ...
    assert response.instrument_types == ["FX_SWAP", "FX_FORWARD"]
