...
from src.services.query_service.app.services.index_price_series import (
    ...
    resolve_index_price_series_response,
)
...
def test_resolve_index_price_series_response_orchestrates_repository_read() -> None:
    observed_at = datetime(2026, 1, 31, 9, 30, tzinfo=UTC)
    request = IndexSeriesRequest(
        as_of_date=date(2026, 1, 31),
        window=IntegrationWindow(
    ...
    async def run_case() -> tuple[object, list[dict[str, object]]]:
        ...
        class Repository:
            async def list_index_price_series(self, **kwargs: object) -> list[SimpleNamespace]:
                calls.append(kwargs)
                return [
                    SimpleNamespace(
                        series_date=date(2026, 1, 30),
        ...
        response = await resolve_index_price_series_response(
