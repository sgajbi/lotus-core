...
def test_resolve_external_hedge_execution_readiness_response_orchestrates_binding_read() -> None:
    async def run_case() -> tuple[object, list[dict[str, object]]]:
        ...
        response = await resolve_external_hedge_execution_readiness_response(
...
def test_resolve_external_hedge_execution_readiness_response_returns_none_without_binding() -> None:
    async def run_case() -> tuple[object, list[dict[str, object]]]:
        calls: list[dict[str, object]] = []
        ...
        response = await resolve_external_hedge_execution_readiness_response(
            ...
            request=ExternalHedgeExecutionReadinessRequest(as_of_date=date(2026, 5, 3)),
        )
        return response, calls
