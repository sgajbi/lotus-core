...
from src.services.query_service.app.services.classification_taxonomy import (
    ...
    resolve_classification_taxonomy_response,
)
...
def test_resolve_classification_taxonomy_response_orchestrates_repository_read() -> None:
    observed_at = datetime(2026, 1, 31, 9, 30, tzinfo=UTC)
    ...
    async def run_case() -> tuple[object, list[dict[str, object]]]:
        ...
        class Repository:
            async def list_taxonomy(self, **kwargs: object) -> list[SimpleNamespace]:
                calls.append(kwargs)
                return [
                    SimpleNamespace(
                        classification_set_id="wm_global_taxonomy_v1",
                        ...
                        dimension_description="Fixed income classification",
                        effective_from=date(2025, 1, 1),
                        effective_to=None,
                        quality_status="accepted",
                        observed_at=observed_at,
        ...
        response = await resolve_classification_taxonomy_response(
    ...
    assert response.product_name == "InstrumentReferenceBundle"
    assert response.records[0].taxonomy_scope == "instrument"
    assert response.records[0].dimension_value == "fixed_income"
    assert response.data_quality_status == "COMPLETE"
    assert calls == [
        {
            "as_of_date": date(2026, 1, 31),
