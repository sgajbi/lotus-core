...
from src.services.query_service.app.services.external_hedge_policy import (
    EXTERNAL_HEDGE_POLICY_BLOCKED_CAPABILITIES,
    EXTERNAL_HEDGE_POLICY_MISSING_FAMILIES,
    build_external_hedge_policy_response,
    resolve_external_hedge_policy_response,
)
...
def _request() -> ExternalHedgePolicyRequest:
    return ExternalHedgePolicyRequest(
...
def test_build_external_hedge_policy_response_fails_closed() -> None:
    response = build_external_hedge_policy_response(
        portfolio_id="PB_SG_GLOBAL_BAL_001",
        binding=_binding(),
        request=_request(),
    )
    ...
    assert response.supportability.missing_data_families == (EXTERNAL_HEDGE_POLICY_MISSING_FAMILIES)
...
def test_resolve_external_hedge_policy_response_orchestrates_binding_read() -> None:
    async def run_case() -> tuple[object, list[dict[str, object]]]:
        ...
        response = await resolve_external_hedge_policy_response(
...
def test_resolve_external_hedge_policy_response_returns_none_without_binding() -> None:
    async def run_case() -> tuple[object, list[dict[str, object]]]:
        calls: list[dict[str, object]] = []
