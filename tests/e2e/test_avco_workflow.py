...
def setup_avco_data(clean_db_module, e2e_api_client: E2EApiClient, poll_db_until):
    ...
    def validation_func(data):
        return data.get("transactions") and len(data["transactions"]) == 3 and next((t for t in data["transactions"] if t["transaction_id"] == "AVCO_SELL_1"), {}).get("realized_gain_loss") is not None  # noqa: E501
